--[[
    ...
    This build: UI is Dashboard-only (Lumexa Scanner title,
    Executor line, Credits, Boss Found / Webhook Sent persistent
    counters). BossManager now alerts each boss/event exactly once
    per server. Boss Found and Webhook Sent counters are persisted
    to file (writefile/readfile) so they survive server hops.
]]
...
BossManager._clearNotified = false -- edge-trigger flag, see onScanResults below
BossManager._alertedThisServer = {} -- boss name -> true, once processed, never re-processed this server

--======================================================================
-- Persistent "Boss Found" counter
--======================================================================
-- A server hop actually ends the current script (Roblox teleports the
-- player into a fresh server instance, which re-runs the script from
-- scratch), so an in-memory counter alone resets on every hop. To make
-- this count survive across hops, it's persisted to a file via the
-- executor's writefile/readfile — supported by Delta, Codex, Synapse X,
-- Script-Ware, Fluxus, and effectively every mainstream executor. If
-- the executor doesn't support file I/O, this quietly falls back to a
-- normal in-memory counter (still correct within one server, just not
-- persistent across hops).
...
local function saveTotalFound()
    local ok, supported = pcall(function() return writefile ~= nil end)
    if not ok or not supported then return end
    pcall(writefile, STATS_FILE, tostring(BossManager._totalFound))
end

BossManager._totalFound = loadTotalFound()
...
local function onScanResults(results)
    ...
    for _, entity in ipairs(results.entities or {}) do
        if isTargetBoss(entity.name) then
            ...
            if not BossManager._alertedThisServer[entity.name] then
                table.insert(newSightings, entity)
            end
        end
    end
    ...
    if #newSightings > 0 then
        BossManager._clearNotified = false
        Logger.Debug("BossManager", string.format("%d new target boss(es) detected", #newSightings))
        notify(newSightings)

        for _, sighting in ipairs(newSightings) do
            -- Marked immediately, before the send even attempts — this
            -- is what makes it a genuine one-time scan per boss/event:
            -- once seen, it is never processed again this server,
            -- regardless of whether the webhook send itself succeeds.
            BossManager._alertedThisServer[sighting.name] = true
    ...
    elseif not BossManager._clearNotified then
        -- Fires once every currently-visible target has already been
        -- alerted (or there were none at all) — a boss that's still
        -- physically in range but already processed no longer blocks
        -- this, since it's been handled and won't be scanned again.
        BossManager._clearNotified = true
        Logger.Info("BossManager", "No new target bosses — clear to hop")
        notifyAllClear()
    end
end
...
function UI._buildDashboardTab()
    ...
    local fields = {
        BossesFound = tab:CreateLabel("Boss Found: " .. tostring(BossManager.GetTotalFound())),
        WebhooksSent = tab:CreateLabel("Webhook Sent: " .. tostring(Webhook.GetTotalSent())),
    }

    -- These counters are backed by BossManager.GetTotalFound() /
    ...
    -- BossManager.lua / Webhook.lua).
    local function refresh()
        pcall(function() fields.BossesFound:Set("Boss Found: " .. tostring(BossManager.GetTotalFound())) end)
