...
local function onScanResults(results)
    ...
    elseif not BossManager._clearNotified then
        -- Fires exactly once per "no active targets" streak — covers
        -- both a boss that was found and then left, AND a server that
        -- had zero target bosses from the very first scan. Previously
        -- only the first case fired this signal, so a server without
        -- any boss at all would scan forever without ever hopping.
        BossManager._clearNotified = true
        Logger.Info("BossManager", "No active target bosses — clear to hop")
        notifyAllClear()
    end
end
