...
Config.BossManager = {
    ...
    AutoFarm = false,
    TargetBosses = {
        "Rip Indra", "Dough King", "Cake Prince", "Soul Reaper",
    },
}
...
Config.Intelligence = {
    ...
    -- migrated from module-local constants, same defaults as before
    AlertsMax = 100,
    BossTrailMax = 10,
    BossHistoryMax = 20,
    EnvironmentHistoryMax = 20,
    EventHistoryMax = 20,
    IslandAppearancesMax = 20,
    ScannerClearDebounceSeconds = 3,
}

-- see HistoryEngine.lua: how many merged timeline entries are kept
Config.HistoryTimelineMax = 150
...
Config.Webhook = {
    ...
    MaxRetries = 3,
    RetryDelaySeconds = 5,
    TimeoutSeconds = 10, -- best-effort, see Webhook.lua sendRequest()
...
-- lower number = shown at more permissive filter settings
...
function Logger.Log(level, module, message)
    ...
    local minLevel = Config.Get("Logger.MinLevel") or "DEBUG"
    local minPriority = LEVEL_PRIORITY[minLevel] or 1
    local thisPriority = LEVEL_PRIORITY[level] or 2
    if thisPriority < minPriority then
        return
    end
...
function Logger.SetMinLevel(level)
    if not LEVEL_PRIORITY[level] then
        return false, "Unknown level: " .. tostring(level)
    end
    Config.Set("Logger.MinLevel", level)
    return true
end

function Logger.GetMinLevel()
    return Config.Get("Logger.MinLevel") or "DEBUG"
end
...
local function onDiscoveryEvent(event)
    ...
    local instance = WorkspaceDetector.ResolveInstance(event.fullPath)
...
local function pushTrail(entry, position, now)
    ...
    if #entry.trail > (Config.Get("Intelligence.BossTrailMax") or 10) then
...
local function pushHistory(entry, record)
    ...
    if #entry.history > (Config.Get("Intelligence.BossHistoryMax") or 20) then
...
local function pushAppearance(entry, appearance)
    ...
    if #entry.appearances > (Config.Get("Intelligence.IslandAppearancesMax") or 20) then
...
-- caches resolved container references by path, so an enabled event's
-- container isn't re-walked from Workspace via FindFirstChild every
-- tick — these containers essentially never move at runtime. Still
-- checks .Parent before reusing so a destroyed/renamed container gets
-- re-resolved rather than silently reused stale.
local containerCache = {}

local function getCachedContainer(pathString)
    local cached = containerCache[pathString]
    if cached and (cached.Parent ~= nil or cached == Workspace) then
        return cached
    end

    local resolved = WorkspaceDetector.ResolveContainer(pathString)
    containerCache[pathString] = resolved
    return resolved
end
...
local function checkAttribute(eventConfig)
    ...
    local container = getCachedContainer(locationsPath)
...
local function checkModelPresence(eventConfig)
    ...
    local container = getCachedContainer(enemiesPath)
...
local function pushHistory(entry, record)
    table.insert(entry.history, record)
    if #entry.history > (Config.Get("Intelligence.EventHistoryMax") or 20) then
        table.remove(entry.history, 1)
    end
end
...
local function pushHistory(historyTable, record)
    ...
    if #historyTable > (Config.Get("Intelligence.EnvironmentHistoryMax") or 20) then
...
local function recompute()
    ...
    StatisticsEngine._snapshot = {
        ...
        scanCount = VerificationEngine.GetScanCount(),
        duplicateCount = VerificationEngine.GetDuplicateCount(),
        averageProcessingDurationMs = VerificationEngine.GetAverageProcessingMs(),
        cacheHits = Cache.GetHits(),
        cacheMisses = Cache.GetMisses(),

        -- Runtime (existing, reused)
        runtimeSeconds = Config.GetUptime(),
    }
end
...
local function rebuild()
    ...
    local maxEntries = Config.Get("HistoryTimelineMax") or 150
    if #merged > maxEntries then
        local trimmed = {}
        local startIndex = #merged - maxEntries + 1
...
--- cached, up to Config.HistoryTimelineMax), oldest first.
...
local function pushAlert(alertType, message, severity)
    ...
    if #AlertEngine._alerts > (Config.Get("Intelligence.AlertsMax") or 100) then
...
-- Capsule
local Capsule = (function()
-- capsule ui
--
-- Structure, padding, corner radius, layout, the intro slide+fade, and
-- the glow pulse tween are unchanged from the original implementation.
-- What's changed since: FPS/Ping/Region tracking and the region HTTP
-- fetch are gone (no longer a performance monitor), the hardcoded
-- 3-message startup cycle is gone (StatusManager now drives what gets
-- shown), dot/glow color is exposed as a setter instead of hardcoded
-- green, text is now fixed white instead of tracking status color, and
-- the type/delete reveal was rebuilt as a per-character pop in/out
-- (fade + scale, via individual character labels) instead of a single
-- label's MaxVisibleGraphemes — matches a reference clip where each
-- character visibly grows in rather than snapping to full size.
--
-- This module only creates UI and exposes primitives — it holds no
-- opinion about WHEN to show WHAT. That orchestration lives in
-- StatusManager, per "no module should directly modify the UI."
...
local TYPE_INTERVAL   = 0.015
local DELETE_INTERVAL = 0.01

local COLORS = {
    Green  = Color3.fromRGB(55, 227, 90),
    Orange = Color3.fromRGB(240, 150, 55),
    Blue   = Color3.fromRGB(70, 160, 240),
    Red    = Color3.fromRGB(235, 80, 80),
}
Capsule.Colors = COLORS

-- Per the reference clip: text always stays white — only the dot carries
-- status color now. Set once below and never touched again.
local TEXT_COLOR = Color3.fromRGB(255, 255, 255)

local dot, glow, textContainer = nil, nil, nil
local initialized = false
-- Tracks what's shown as actual characters + their live Instances, since
-- there's no single label.Text to read back anymore.
local currentText = ""
local charLabels = {} -- ordered list of { label = TextLabel, scale = UIScale }
...
function Capsule.Init()
    ...
    screenGui.Name = "CapsuleNotification"
    ...
    capsule.Position = UDim2.new(0.5, 0, 0, -36)
    capsule.AutomaticSize = Enum.AutomaticSize.X
    capsule.Size = UDim2.new(0, 0, 0, 32)
    capsule.BackgroundColor3 = Color3.fromRGB(10, 14, 24)
    ...
    local capsulePadding = Instance.new("UIPadding")
    capsulePadding.PaddingLeft = UDim.new(0, 11)
    capsulePadding.PaddingRight = UDim.new(0, 16)
    capsulePadding.PaddingTop = UDim.new(0, 7)
    capsulePadding.PaddingBottom = UDim.new(0, 7)
    capsulePadding.Parent = capsule
    ...
    dotHolder.Size = UDim2.new(0, 9, 0, 9)
    dotHolder.LayoutOrder = 1
    dotHolder.Parent = capsule
    ...
    -- pulse: unchanged from the original — tweens Size/Transparency
    -- only, never BackgroundColor3, so SetDotColor() never disturbs it
    task.spawn(function()
        while glow.Parent do
            TweenService:Create(glow, TweenInfo.new(0.7, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
                { Size = UDim2.new(0, 15, 0, 15), BackgroundTransparency = 0.75 }):Play()
            task.wait(0.7)
            TweenService:Create(glow, TweenInfo.new(0.7, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
                { Size = UDim2.new(0, 9, 0, 9), BackgroundTransparency = 0.4 }):Play()
            task.wait(0.7)
        end
    end)

    textContainer = Instance.new("Frame")
    textContainer.Name = "MessageText"
    textContainer.BackgroundTransparency = 1
    textContainer.AutomaticSize = Enum.AutomaticSize.X
    textContainer.Size = UDim2.new(0, 0, 1, 0)
    textContainer.LayoutOrder = 2
    textContainer.Parent = capsule

    local charLayout = Instance.new("UIListLayout")
    charLayout.FillDirection = Enum.FillDirection.Horizontal
    charLayout.VerticalAlignment = Enum.VerticalAlignment.Center
    charLayout.Padding = UDim.new(0, 0)
    charLayout.SortOrder = Enum.SortOrder.LayoutOrder
    charLayout.Parent = textContainer

    task.wait(0.3)
    TweenService:Create(capsule, TweenInfo.new(0.35, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
        { GroupTransparency = 0, Position = UDim2.new(0.5, 0, 0, -24) }):Play()
end

local COLOR_TWEEN_INFO = TweenInfo.new(0.18, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)

--- Tweens dot + glow to the new color (never snaps instantly — the
--- reference clip shows the dot cross-fading while the old message is
--- still on screen). Label color is untouched: text stays fixed white,
--- only the dot carries status color now.
function Capsule.SetDotColor(colorName)
    local color = COLORS[colorName]
    if not color or not dot then return end
    TweenService:Create(dot, COLOR_TWEEN_INFO, { BackgroundColor3 = color }):Play()
    TweenService:Create(glow, COLOR_TWEEN_INFO, { BackgroundColor3 = color }):Play()
end

local CHAR_POP_IN_INFO  = TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
local CHAR_POP_OUT_INFO = TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.In)

--- Codepoint-safe split, not a byte split — every status string today is
--- plain ASCII so this is equivalent to byte-splitting either way, but
--- it won't tear a multi-byte character in half if one ever shows up.
local function toChars(msg)
    local chars = {}
...
local function newCharLabel(char, order)
    local charLabel = Instance.new("TextLabel")
    charLabel.BackgroundTransparency = 1
    charLabel.AutomaticSize = Enum.AutomaticSize.X
    charLabel.Size = UDim2.new(0, 0, 1, 0)
    charLabel.LayoutOrder = order
    charLabel.Font = Enum.Font.GothamMedium
    charLabel.TextSize = 13
    charLabel.TextColor3 = TEXT_COLOR
    ...
    return charLabel, scale
end

function Capsule.GetCurrentText()
    return currentText
end

--- Types the message in one character at a time; each new character
--- pops in (fades + grows from 0.4x scale) instead of snapping straight
--- to full size the way a single label's MaxVisibleGraphemes did.
function Capsule.TypeMessage(msg)
    if not textContainer then return end
    currentText = msg
    local chars = toChars(msg)
    for i, char in ipairs(chars) do
        local charLabel, scale = newCharLabel(char, #charLabels + 1)
        table.insert(charLabels, { label = charLabel, scale = scale })
        TweenService:Create(charLabel, CHAR_POP_IN_INFO, { TextTransparency = 0 }):Play()
        TweenService:Create(scale, CHAR_POP_IN_INFO, { Scale = 1 }):Play()
        task.wait(TYPE_INTERVAL)
    end
end

--- Erases from the end, one character at a time; each removed character
--- pops out (fades + shrinks) instead of vanishing instantly. Waits out
--- the last pop-out before returning so TypeMessage never starts adding
--- new characters into the same container while old ones are still
--- mid-fade (that would collide on LayoutOrder and jumble the two).
function Capsule.DeleteCurrentMessage()
    if not textContainer then return end
    for i = #charLabels, 1, -1 do
        local entry = charLabels[i]
        charLabels[i] = nil
        TweenService:Create(entry.label, CHAR_POP_OUT_INFO, { TextTransparency = 1 }):Play()
        TweenService:Create(entry.scale, CHAR_POP_OUT_INFO, { Scale = 0.4 }):Play()
        task.delay(CHAR_POP_OUT_INFO.Time, function()
            entry.label:Destroy()
        end)
        task.wait(DELETE_INTERVAL)
    end
    currentText = ""
    task.wait(CHAR_POP_OUT_INFO.Time) -- let the last character actually finish fading before we return
end

--- Instant clear, no animation - only used when a high-priority status
--- interrupts an in-progress type/delete cycle, to avoid leaving a
--- half-typed fragment on screen while the new message starts.
function Capsule.ClearInstant()
    if not textContainer then return end
    for _, entry in ipairs(charLabels) do
        entry.label:Destroy()
    end
    charLabels = {}
    currentText = ""
...
-- StatusManager
local StatusManager = (function()
-- status manager
--
-- The only module anything else should call for UI status. Owns:
-- text updates, dot color, the type/delete transition, and priority
-- handling. Nothing else touches Capsule directly.
--
-- Two ways messages reach here:
--   1. Generic states with a fixed message, derived automatically by
--      subscribing to the EXISTING Status.lua module (Status.Subscribe)
--      — zero changes needed to ServerHop/Webhook/Main, since they
--      already call Status.Set(...) at exactly the right moments.
--   2. Dynamic messages (which boss, which module, why a webhook
--      failed) that no fixed State enum value can carry — these need
--      one small, explicit StatusManager.SetStatus(...) call inserted
--      at the specific site where that information is available
--      (NotificationManager, Webhook's failure path, ScannerCore's
--      detector-isolation retry).


local StatusManager = {}

local LAST_RUN_FILE = "Lumexa_LastRun.txt"
local WELCOME_BACK_THRESHOLD_SECONDS = 3600

-- exact-match messages -> dot color
local EXACT_COLOR = {
    ["Welcome Back"] = "Green",
    ["Initializing Lumexa..."] = "Green",
    ["Scanner Ready"] = "Green",
    ["Scanning..."] = "Green",
    ["Mirage Island Detected"] = "Green",
    ["Prehistoric Island Detected"] = "Green",
    ["Legendary Dealer Detected"] = "Green",
    ["Full Moon Detected"] = "Green",
    ["Webhook Sent"] = "Green",
    ["Scanner Recovered"] = "Green",
    ["Server Hopping..."] = "Orange",
    ["Searching Server..."] = "Orange",
    ["Waiting..."] = "Orange",
    ["Loading Scanner Modules..."] = "Blue",
    ["Connecting Services..."] = "Blue",
...
-- messages with a dynamic suffix -> matched by prefix
local PREFIX_COLOR = {
    { prefix = "Boss Detected", color = "Green" },
    { prefix = "Fixing Error:", color = "Red" },
    { prefix = "Webhook Failed", color = "Red" },
}

-- Exact, fixed-string high-priority messages. "Boss Detected" /
-- "Webhook Failed" / "Fixing Error(s)" all carry a dynamic suffix so
-- they stay prefix-matched below instead of living in this set.
local HIGH_PRIORITY_EXACT = {
    ["Mirage Island Detected"] = true,
    ["Prehistoric Island Detected"] = true,
    ["Legendary Dealer Detected"] = true,
    ["Full Moon Detected"] = true,
}

-- Prefix match on purpose: "Fixing Error" also matches "Fixing Errors...",
-- so singular and plural both land here with one entry.
local HIGH_PRIORITY_PREFIXES = { "Boss Detected", "Webhook Failed", "Fixing Error" }

local function colorFor(message)
    if EXACT_COLOR[message] then
        return EXACT_COLOR[message]
    end
    for _, rule in ipairs(PREFIX_COLOR) do
        if message:sub(1, #rule.prefix) == rule.prefix then
            return rule.color
        end
    end
    return "Green"
...
local function isHighPriority(message)
    if HIGH_PRIORITY_EXACT[message] then
        return true
    end
    for _, prefix in ipairs(HIGH_PRIORITY_PREFIXES) do
...
-- Queue-based orchestration. Two FIFOs instead of one shared slot, so
-- a high-priority message can jump ahead of whatever's queued behind
-- it WITHOUT ever cancelling a transition that's already animating —
-- nothing below ever calls task.cancel, so "finish the transition
-- smoothly, then type the next message" always holds, and a normal
-- multi-step flow (Sending Webhook -> Webhook Sent -> Server Hopping
-- -> ...) never has a middle step silently dropped.
local normalQueue = {}
local priorityQueue = {}
local workerStarted = false

local IDLE_MESSAGE = "Scanning..."
-- Minimum time a finished message stays fully visible before the next
-- queued one starts erasing it — the readability floor from the task.
-- A newer high-priority arrival can cut this short; normal-priority
-- messages never cut each other's hold short.
local HOLD_SECONDS = 1.2
-- General safety net only, for any state that isn't explicitly reset
-- elsewhere: how long both queues must sit empty before falling back
-- to the idle message. Kept long enough that a normal webhook
-- round-trip doesn't cause a flicker back to "Scanning..." mid-flow.
local IDLE_DEBOUNCE_SECONDS = 6
local MAX_NORMAL_QUEUE = 4 -- safety cap; normal traffic should never realistically reach this

local function enqueue(queue, message, cap)
    if queue[#queue] == message then
        return -- de-dupe: never queue the same message twice in a row
    end
    table.insert(queue, message)
    if cap and #queue > cap then
        table.remove(queue, 1) -- drop the oldest rather than grow without bound
    end
end

--- One full delete-old/type-new cycle. Dot color starts tweening
--- immediately, before the old message even starts erasing — matches
--- the reference clip, where the dot is already mid-fade to the new
--- color while the old text is still fully on screen. Text itself
--- stays fixed white throughout (see Capsule.SetDotColor).
local function runTransition(message)
    Capsule.SetDotColor(colorFor(message))
    if Capsule.GetCurrentText() ~= "" then
        Capsule.DeleteCurrentMessage()
    end
    Capsule.TypeMessage(message)
end

--- Single perpetual worker. Priority queue always drains first; the
--- normal queue only advances once it's empty. Never interrupts a
--- runTransition already in progress — only the hold AFTER one can be
--- cut short, and only by a high-priority arrival.
local function worker()
    ...
    while true do
        ...
        if #priorityQueue > 0 then
            message = table.remove(priorityQueue, 1)
        elseif #normalQueue > 0 then
            message = table.remove(normalQueue, 1)
        end

        if message then
            emptySince = nil
            runTransition(message)

            local held = 0
            while held < HOLD_SECONDS do
                task.wait(0.1)
                held = held + 0.1
                if #priorityQueue > 0 then
                    break -- something urgent is waiting, don't make it wait out our hold too
                end
            end
        else
            if not emptySince then
                emptySince = os.clock()
            elseif (os.clock() - emptySince) >= IDLE_DEBOUNCE_SECONDS
                and Capsule.GetCurrentText() ~= IDLE_MESSAGE then
                runTransition(IDLE_MESSAGE)
                emptySince = os.clock()
            end
            task.wait(0.2)
        end
...
function StatusManager.SetStatus(message)
    if type(message) ~= "string" or message == "" then
        return
    end

    if isHighPriority(message) then
        enqueue(priorityQueue, message, nil)
    else
        enqueue(normalQueue, message, MAX_NORMAL_QUEUE)
    end
...
-- generic states derived automatically from the existing Status module
local STATE_TO_MESSAGE = {
    [Status.States.SEARCHING] = "Scanning...",
    [Status.States.SENDING_WEBHOOK] = "Sending Webhook...",
    [Status.States.WEBHOOK_SENT] = "Webhook Sent",
    [Status.States.WAITING] = "Waiting...",
    [Status.States.SERVER_HOPPING] = "Server Hopping...",
    [Status.States.CONNECTED] = "Scanner Ready",
}

local function onStatusChanged(state)
    local message = STATE_TO_MESSAGE[state]
    if message then
        StatusManager.SetStatus(message)
    end
    -- states with no mapping here (IDLE, BOSS_FOUND, NO_BOSS_FOUND,
    -- TELEPORT_FAILED, RETRYING) are intentionally not mapped:
    -- BOSS_FOUND needs the boss name (handled explicitly at the call
    -- site instead, see NotificationManager), and RETRYING is reused
    -- for two different underlying processes in Status.lua (webhook
    -- retry and teleport retry) with no way to disambiguate here.
end

local function loadLastRunAndCheckWelcomeBack()
    local ok, supported = pcall(function() return isfile ~= nil and readfile ~= nil and writefile ~= nil end)
    local shouldWelcomeBack = false

    if ok and supported then
        local existsOk, exists = pcall(isfile, LAST_RUN_FILE)
        if existsOk and exists then
            local readOk, content = pcall(readfile, LAST_RUN_FILE)
            local lastRun = readOk and tonumber(content)
            if lastRun and (os.time() - lastRun) >= WELCOME_BACK_THRESHOLD_SECONDS then
                shouldWelcomeBack = true
            end
        end
        pcall(writefile, LAST_RUN_FILE, tostring(os.time()))
    end

    return shouldWelcomeBack
end

--- Runs once, at startup. Blocking (uses the same task.wait-driven
--- animation as everything else) — Main.lua calls this before moving
--- on to starting the actual scanner.
function StatusManager.RunStartupSequence()
    Capsule.Init()

    if loadLastRunAndCheckWelcomeBack() then
        StatusManager.SetStatus("Welcome Back")
        task.wait(2)
    end

    StatusManager.SetStatus("Initializing Lumexa...")
    task.wait(0.4)
    StatusManager.SetStatus("Loading Scanner Modules...")
    task.wait(0.4)
    StatusManager.SetStatus("Connecting Services...")
    task.wait(0.4)
    StatusManager.SetStatus("Scanner Ready")
    task.wait(0.4)
    StatusManager.SetStatus("Scanning...")
end

Status.Subscribe(onStatusChanged)
...
--- Resolves a GetFullName()-style path ("Workspace.Enemies.Don Swan")
--- back to a live Instance, starting from a Service rather than
--- Workspace specifically. Consolidated here from NPCIntelligence and
--- IslandIntelligence, which had byte-identical copies of this.
--- Known limitation: a name containing a literal "." breaks the naive
--- split — Roblox's own GetFullName() escapes that with bracket
--- notation, which this doesn't parse. Not a concern for Blox Fruits
--- NPC/boss/island names.
function WorkspaceDetector.ResolveInstance(fullPath)
    local parts = {}
...
function Cache.GetHits()
    return Cache._hits
end

function Cache.GetMisses()
    return Cache._misses
end
...
local function sendRequest(url, payload)
    ...
    -- Timeout is best-effort: most request()-style executor APIs
    -- accept this field, but not all actually enforce it — harmless
    -- if ignored, useful where supported.
    local callOk, response = pcall(httpRequest, {
        ...
        Body = encoded,
        Timeout = Config.Get("Webhook.TimeoutSeconds") or 10,
    })
...
local function processItem(item)
    ...
    if item.attempts >= maxRetries then
        ...
        StatusManager.SetStatus("Webhook Failed: " .. tostring(err))
...
-- NotificationManager
local NotificationManager = (function()
-- notification manager
--
-- Sits between VerificationEngine (acts as the EventDispatcher — every
-- detector already routes exclusively through VerificationEngine.Submit,
-- confirmed via grep, zero bypasses) and Webhook (the WebhookDispatcher).
-- Extracted from what used to be VerificationEngine's own internal
-- dispatch() function — same logic, moved here so notification-building
-- is its own separable concern rather than living inside the
-- confidence-verification module.


local NotificationManager = {}

-- same pattern already used identically in AlertEngine/EventIntelligence/
-- TimelineEngine — distinguishes "Boss Detected: X" phrasing from the
-- "X Detected" phrasing everything else uses
local function isTargetBoss(name)
    local targets = Config.Get("BossManager.TargetBosses") or {}
    for _, targetName in ipairs(targets) do
        if targetName == name then return true end
...
--- Records the dedup entry, builds the Discord embed fields, and
--- hands off to Webhook. Returns (ok, err) exactly as the old
--- VerificationEngine.dispatch() did — same signature, same behavior.
function NotificationManager.Notify(candidate)
    local jobId = "unknown"
    pcall(function() jobId = game.JobId end)
    ...
    local name = candidate.displayName or candidate.eventKey
    if isTargetBoss(candidate.eventKey) then
        StatusManager.SetStatus("Boss Detected: " .. name)
    else
        StatusManager.SetStatus(name .. " Detected")
    end
...
VerificationEngine._scanCount = 0
VerificationEngine._duplicateCount = 0
VerificationEngine._processingTimeTotalMs = 0
VerificationEngine._processingTimeCount = 0

function VerificationEngine.GetTotalFound()
    return VerificationEngine._totalFound
end

function VerificationEngine.GetScanCount()
    return VerificationEngine._scanCount
end

function VerificationEngine.GetDuplicateCount()
    return VerificationEngine._duplicateCount
end

function VerificationEngine.GetAverageProcessingMs()
    if VerificationEngine._processingTimeCount == 0 then return nil end
    return VerificationEngine._processingTimeTotalMs / VerificationEngine._processingTimeCount
end
...
--- Delegates the actual notification-building to NotificationManager
--- (extracted out of this file — same logic, separate concern).
local function dispatch(candidate)
    local ok, err = NotificationManager.Notify(candidate)
...
function VerificationEngine.Submit(candidate)
    ...
    VerificationEngine._scanCount = VerificationEngine._scanCount + 1
    ...
    if not confirmOk then
        if confirmReason == "Already alerted this server" then
            VerificationEngine._duplicateCount = VerificationEngine._duplicateCount + 1
        end
    ...
    VerificationEngine._processingTimeTotalMs = VerificationEngine._processingTimeTotalMs + confirmationMs
    VerificationEngine._processingTimeCount = VerificationEngine._processingTimeCount + 1
...
-- MoonDetector
local MoonDetector = (function()
-- full moon


local MoonDetector = {}

MoonDetector._connection = nil
MoonDetector._lastState = false
...
local function checkAndSubmit()
    ...
    if active and not MoonDetector._lastState then
        VerificationEngine.Submit({
            eventKey = "FullMoon",
    ...
    MoonDetector._lastState = active
end

function MoonDetector.Start()
    if MoonDetector._connection then
        return true
    end

    if not Lighting then
        Logger.Error("MoonDetector", "Lighting service unavailable")
        return false
    end

    local eventConfig = Config.Get("SeaEvents.FullMoon") or {}
    if eventConfig.StateAttributeName then
        MoonDetector._connection = WorkspaceDetector.WatchAttribute(
            Lighting, eventConfig.StateAttributeName, function() checkAndSubmit() end
        )
    else
        MoonDetector._connection = WorkspaceDetector.WatchProperty(
            Lighting, "ClockTime", function() checkAndSubmit() end
        )
    end

    Logger.Info("MoonDetector", "Watching Lighting for Full Moon")
    return true
end

function MoonDetector.Stop()
    if MoonDetector._connection then
        MoonDetector._connection:Disconnect()
        MoonDetector._connection = nil
    end
end

--- Fallback sweep entry point (see ARCHITECTURE.md §9).
function MoonDetector.FallbackSweep()
    checkAndSubmit()
end
...
-- IslandDetector
local IslandDetector = (function()
-- island events (Mirage Island, Prehistoric Island)


local IslandDetector = {}

IslandDetector._connections = {}
...
local function watchLocationEvent(eventKeyConfigPath, eventKey, displayName)
    ...
    if not locationName then
        Logger.Warn("IslandDetector", eventKey .. " enabled but LocationName not set")
        return
    end
    ...
    if connection then
        table.insert(IslandDetector._connections, connection)
    end
end

function IslandDetector.Start()
    ...
    Logger.Info("IslandDetector", "Started (" .. #IslandDetector._connections .. " active watcher(s) — events without confirmed [verify] fields are skipped)")
end

function IslandDetector.Stop()
    for _, connection in ipairs(IslandDetector._connections) do
        pcall(function() connection:Disconnect() end)
    end
    IslandDetector._connections = {}
end

function IslandDetector.FallbackSweep()
...
local function checkClearToHop()
    ...
    local debounceSeconds = Config.Get("Intelligence.ScannerClearDebounceSeconds") or 3
    if (os.clock() - emptySince) >= debounceSeconds and not ScannerCore._wasClear then
        ScannerCore._wasClear = true
        Logger.Info("ScannerCore", "No active bosses for " .. debounceSeconds .. "s — clear to hop")
        notifyAllClear()
    end
end

local DETECTOR_START_MAX_RETRIES = 3
local DETECTOR_START_RETRY_DELAY_SECONDS = 2

--- Starts a single detector in isolation. If Start() throws (e.g. a
--- transient failure while the game world is still streaming in),
--- retries a few times before giving up on THAT detector specifically
--- — a startup failure in one must never prevent the others from
--- starting, which the previous unguarded sequential calls did not
--- protect against.
local function startDetectorIsolated(name, detector)
    for attempt = 1, DETECTOR_START_MAX_RETRIES do
        local ok, err = pcall(detector.Start)
        if ok then
            if attempt > 1 then
                StatusManager.SetStatus("Scanner Recovered")
            end
            return true
        end

        Logger.Error("ScannerCore", string.format(
            "%s failed to start (attempt %d/%d): %s",
            name, attempt, DETECTOR_START_MAX_RETRIES, tostring(err)
        ))
        StatusManager.SetStatus("Fixing Error: " .. name)

        if attempt < DETECTOR_START_MAX_RETRIES then
            task.wait(DETECTOR_START_RETRY_DELAY_SECONDS)
        end
    end

    Logger.Error("ScannerCore", name .. " failed to start after " .. DETECTOR_START_MAX_RETRIES .. " attempts — continuing without it")
    return false
end
...
function ScannerCore.Start()
    ...
    startDetectorIsolated("BossDetector", BossDetector)
    startDetectorIsolated("MoonDetector", MoonDetector)
    startDetectorIsolated("IslandDetector", IslandDetector)
        ...
        while ScannerCore._running do
            ...
            if Config.Get("Detection.FallbackSweepEnabled") then
                ...
                if (os.clock() - lastFallbackSweep) >= sweepInterval then
                    ...
                    pcall(MoonDetector.FallbackSweep)
                    pcall(IslandDetector.FallbackSweep)
...
-- Fix 4: exponential backoff for fetchServerPage failures. Literal
-- sequence as specified — 3s, 6s, 12s, then capped at 30s for any
-- further consecutive failure — not continued doubling past 12s.
-- Resets to the front of the sequence on the next successful fetch.
local BACKOFF_SEQUENCE = { 3, 6, 12, 30 }
local fetchFailureStreak = 0

--- Fetches a single page of public server instances for the current place.
--- Returns (servers, nextCursor) or (nil, nil) on failure. Retries
--- internally with the backoff sequence above before finally giving up.
local function fetchServerPage(cursor)
    ...
    -- bounded so a persistently broken HttpService can't hang this
    -- forever — one more attempt than the backoff table has entries
    local maxAttempts = #BACKOFF_SEQUENCE + 1
    ...
    for attempt = 1, maxAttempts do
        ...
        if attempt < maxAttempts then
            fetchFailureStreak = fetchFailureStreak + 1
            local waitSeconds = BACKOFF_SEQUENCE[math.min(fetchFailureStreak, #BACKOFF_SEQUENCE)]
            Logger.Warn("ServerHop", string.format(
                "Fetch attempt %d/%d failed, backing off %ds before retry",
...
-- Fix 2: Fisher-Yates (Durstenfeld variant) — standard, unbiased shuffle.
...
-- How many of the best-by-free-slots candidates to randomize among.
-- Not exposed via Config — the task specified a fixed 5-10 range, not
-- a new tunable, and existing config schema should stay unchanged.
local SHORTLIST_SIZE = 8
...
local function findBestServer()
    ...
    repeat
        ...
        if not servers then
            break -- fetch failed even after internal backoff retries, stop this attempt
        end
    ...
    if #candidates == 0 then
        return nil, pagesScanned
    end

    -- Fix 2: shuffle first so a stable sort breaks free-slot ties
    -- randomly (not by whatever order the API happened to return
    -- them in), then keep only the best SHORTLIST_SIZE, then pick
    -- randomly among those — variety, without ever picking a
    -- meaningfully worse-than-necessary server. Verified with an
    -- isolated 2000-trial simulation before wiring this in: zero
    -- picks ever fell outside the top 8 by free slots, and each of
    -- the top 8 got picked roughly evenly.
    shuffle(candidates)
    table.sort(candidates, function(a, b)
        return (a.maxPlayers - a.playing) > (b.maxPlayers - b.playing)
    end)

    local shortlistSize = math.min(SHORTLIST_SIZE, #candidates)
    local chosenIndex = math.random(1, shortlistSize)
    local bestServer = candidates[chosenIndex]
...
-- Fix 3: cooldown + bounded internal retry when nothing is found.
-- Not part of the literal request but added deliberately: without
-- some upper bound, an empty/over-restrictive server browser would
-- otherwise retry every 10s forever and never let HopOnce return.
-- 30 attempts = up to ~5 minutes before giving up and reporting
-- failure to the caller.
local NO_SERVER_MAX_ATTEMPTS = 30
local NO_SERVER_COOLDOWN_SECONDS = 10
...
function ServerHop.HopOnce()
    ...
    local crashed, ok, err = pcall(function()
        local target, pagesScanned

        for attempt = 1, NO_SERVER_MAX_ATTEMPTS do
            target, pagesScanned = findBestServer()
            if target then
                break
            end

            Logger.Warn("ServerHop", string.format(
                "No suitable server found after scanning %d page(s) (attempt %d/%d)",
                pagesScanned or 0, attempt, NO_SERVER_MAX_ATTEMPTS
            ))

            if attempt < NO_SERVER_MAX_ATTEMPTS and Config.Get("ServerHop.Enabled") then
                task.wait(NO_SERVER_COOLDOWN_SECONDS)
            end
        end

        if not target then
            return false, "No suitable server found after " .. NO_SERVER_MAX_ATTEMPTS .. " attempts"
        end
        ...
        -- Fix 1: mark visited as soon as a target is chosen, BEFORE
        -- attempting teleport — so a failed teleport doesn't leave
        -- this same unreachable server eligible to be picked again
        -- next attempt.
    ...
    if not crashed then
        ...
        StatusManager.SetStatus("Scanning...") -- hop attempt is over, resume normal display
        return false, "Internal error: " .. tostring(ok)
    end

    if not ok then
        Logger.Error("ServerHop", tostring(err))
        StatusManager.SetStatus("Scanning...") -- gave up hopping, still on this server — don't strand the capsule on "Server Hopping..."
        return false, err
    end
...
-- ServerHopManager
local ServerHopManager = (function()
-- server hop manager
--
-- thin wrapper, not a reimplementation. ServerHop.lua already owns
-- server-listing, candidate selection, visited-JobId history (with
-- TTL pruning), and the actual teleport call — none of that logic is
-- duplicated here. This module exists purely to give the scanner side
-- a clean, narrow API ("request the next server") instead of reaching
-- into ServerHop's own module directly, isolating "a move should
-- happen now" from "how a move actually happens."


local ServerHopManager = {}

--- Requests a hop after the configured delay (Config.ServerHop.HopDelay).
--- This is what the scan flow should call when nothing was detected.
function ServerHopManager.RequestNextServer()
    if not ServerHopManager.IsEnabled() then
        return false, "ServerHop is disabled in config"
    end
    return ServerHop.HopWithDelay()
end

--- Requests an immediate hop attempt, bypassing the configured delay.
function ServerHopManager.RequestNextServerNow()
    return ServerHop.HopOnce()
end

function ServerHopManager.IsEnabled()
    return Config.Get("ServerHop.Enabled") or false
end

function ServerHopManager.GetVisitedCount()
    return ServerHop.GetVisitedCount()
end

--- Marks a JobId as visited without going through an actual hop
--- (e.g. learned from another source). Delegates to ServerHop, which
--- already applies the safety cap and TTL pruning on this set.
function ServerHopManager.MarkVisited(jobId)
    return ServerHop.MarkVisited(jobId)
end
...
ScannerCore.SubscribeAllClear(function()
    if Main._started and ServerHopManager.IsEnabled() then
        ...
        task.spawn(function()
            ...
            Utilities.SafeCall(ServerHopManager.RequestNextServer, "Main")
        end)
    end
end)
...
--[[
    Startup sequence:
        ...
        Scanner -> ScannerCore starts BossDetector, MoonDetector,
                   IslandDetector — all event-driven (see
...
function Main.Start()
    ...
    Utilities.SafeCall(StatusManager.RunStartupSequence, "Main")
    ...
    -- Stage: Hopper
    if ServerHopManager.IsEnabled() then
