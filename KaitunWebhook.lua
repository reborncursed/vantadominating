...
-- capsule ui
--
-- Reused near-verbatim from the original uploaded implementation.
-- Structure, padding, corner radius, layout, the intro slide+fade, the
-- glow pulse tween, and the type/delete character-reveal animation are
-- all unchanged — same values, same easing, same timing. What changed:
-- FPS/Ping/Region tracking and the region HTTP fetch are gone (no
-- longer a performance monitor), the hardcoded 3-message startup cycle
-- is gone (StatusManager now drives what gets shown), and the dot/glow
-- color is exposed as a setter instead of being hardcoded green.
--
...
local TYPE_INTERVAL   = 0.06
local DELETE_INTERVAL = 0.04
...
local dot, glow, label = nil, nil, nil
...
function Capsule.Init()
    ...
    label.TextSize = 13
    label.TextColor3 = Color3.fromRGB(85, 220, 120)
...
--- Sets dot + glow color instantly. Only touches BackgroundColor3, so
--- the pulse tween (Size/Transparency) is never interrupted by this.
...
--- Same character-reveal animation as the original, unchanged timing.
...
--- Same reverse character-erase animation as the original, unchanged timing.
function Capsule.DeleteCurrentMessage()
    if not label then return end
    local msg = label.Text
    for i = #msg, 0, -1 do
        label.MaxVisibleGraphemes = i
...
local EXACT_COLOR = {
    ...
    ["Server Hopping..."] = "Yellow",
    ["Searching Server..."] = "Yellow",
    ["Waiting..."] = "Yellow",
...
-- per the task: these three categories may interrupt an in-progress
-- transition; everything else waits its turn
...
local busy = false
local pending = nil -- only the most recent queued message survives, not a growing backlog
...
function StatusManager.SetStatus(message)
    ...
    if busy then
        pending = message -- overwrites any earlier pending message, by design
        return
    end
