-- ============================================================
-- NEMESIS X PREMIUM EDITION v2
-- Sacred UI Foundation | by CursedExility
-- Responsive | Progression Managers | 70+ Features
-- ============================================================
...
for _, old in ipairs(playerGui:GetChildren()) do
    if old.Name == "NX_UI" or old.Name == "NX_Toggle" then old:Destroy() end
end
...
-- ============================================================
-- TOP BAR (clean: title center, controls right)
-- ============================================================
...
-- ============================================================
-- SACRED UI HELPERS (Preserved exactly)
-- ============================================================
...
-- addToggleElement (Sacred exact + active counter)
...
-- Toggle row inside auto-card
local function cardToggle(card, labelText, callback)
    local state = false
...
-- Stat row
local function cardStat(card, labelText, initVal)
    local frame = Instance.new("Frame", card)
    frame.Size             = UDim2.new(1, -14, 0, 18)
    frame.BackgroundTransparency = 1
    frame.ZIndex           = 6
...
-- Card dropdown
local function cardDD(card, labelText, optionText)
    local frame = Instance.new("Frame", card)
    frame.Size             = UDim2.new(1, -14, 0, 22)
    frame.BackgroundTransparency = 1
    frame.ZIndex           = 6
...
-- Card slider
local function cardSlider(card, labelText, min, max, default, callback)
    local frame = Instance.new("Frame", card)
    frame.Size             = UDim2.new(1, -14, 0, 30)
    frame.BackgroundTransparency = 1
    frame.ZIndex           = 6
    ...
    local function update(input)
        local pct = math.clamp(
            (input.Position.X - bg.AbsolutePosition.X) / bg.AbsoluteSize.X, 0, 1)
        fill.Size  = UDim2.new(pct, 0, 1, 0)
        local val  = math.round(min + pct * (max - min))
        valLbl.Text = tostring(val)
        if callback then callback(val) end
...
local function cardBtn(card, text, callback)
...
-- Progress bar
local function cardProgress(card, labelText, current, total)
    local frame = Instance.new("Frame", card)
    ...
    countLbl.Text            = tostring(current) .. " / " .. tostring(total)
    countLbl.Font            = Enum.Font.GothamBold
    countLbl.TextSize        = 9
    countLbl.TextColor3      = C.White
    countLbl.Size            = UDim2.new(0.4, 0, 0, 12)
    ...
    local pct = math.clamp(current / math.max(total, 1), 0, 1)
...
local function cardBtnRow(card, btns)
    ...
    for i, info in ipairs(btns) do
        local w = info.w or math.floor((1 / #btns) * 100) / 100
        local b = Instance.new("TextButton", row)
        b.Size             = UDim2.new(w, -(4*(#btns-1)/#btns), 1, 0)
...
local dispVal = cardStat(infoCard, "Display Name",   lp.DisplayName)
...
cardStat(infoCard, "Version",        "v2.0 Premium")
...
cardBtnRow(actCard, {
    ...
    {text="📋 Copy Key",  cb=function() pcall(function() setclipboard(tostring(getgenv and getgenv().NX_Key or "")) end) end},
})
cardBtnRow(actCard, {
    {text="💾 Save Config"},
    {text="📂 Load Config"},
...
local nLbl2 = Instance.new("TextLabel", homeNotifCard)
nLbl2.Text             = "ℹ Sea: " .. seaName .. "  |  Executor: " .. executorName
nLbl2.Font             = Enum.Font.Gotham
nLbl2.TextSize         = 9
nLbl2.TextColor3       = C.Gray
nLbl2.Size             = UDim2.new(1, -14, 0, 16)
nLbl2.BackgroundTransparency = 1
nLbl2.TextXAlignment   = Enum.TextXAlignment.Left
nLbl2.ZIndex           = 6

-- Updates card
local updCard = makeCard(hp, "🔄 Updates")
cardStat(updCard, "Current Version", "v2.0")
cardStat(updCard, "Latest Version",  "v2.0")
cardStat(updCard, "Update Status",   "✔ Up to date")

-- ============================================================
-- ── AUTO FARM PRO PAGE ──
-- ============================================================
local fp = pages["Auto Farm"]
pageHeader(fp, "🌾 Auto Farm Pro — Farm Manager")
...
local farmProgress = cardStat(fmCard, "Progress",       "0%")
...
local _, levCount, levBar = cardProgress(levCard, "Progress", 0, 100)
...
-- Sea Beast Manager
local sbCard = makeCard(sep2, "🌊 Sea Beast Manager")
...
-- ============================================================
-- ── ITEMS PAGE — Progression Managers ──
-- ============================================================
...
local _, yamaCount, _ = cardProgress(yamaCard, "Elite Kills", 0, 30)
...
local _, tushCount, _ = cardProgress(tushCard, "Progress", 0, 100)
...
local _, cdkCount, _ = cardProgress(cdkCard, "Progress", 0, 100)
...
local _, sgCount, _ = cardProgress(sgCard, "Progress", 0, 100)
cardSpacer(sgCard, 2)
...
local espFruitCard = makeCard(ep, "Fruit & Chest")
cardToggle(espFruitCard, "Fruit ESP")
cardToggle(espFruitCard, "Chest ESP")

local espSeaCard = makeCard(ep, "Sea & Island")
cardToggle(espSeaCard, "Leviathan ESP")
cardToggle(espSeaCard, "Island ESP")
...
cardDD(espSetCard, "Color", "Accent")
...
local notifFeed = makeCard(np, "Notification Feed")
local feedLbl = Instance.new("TextLabel", notifFeed)
...
-- Theme Settings (moved from top bar)
...
print("[NX Premium v2] ✔ Loaded | Sea: " .. seaName .. " | Executor: " .. executorName)
