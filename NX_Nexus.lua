...
for _, old in ipairs(playerGui:GetChildren()) do
    ...
    or old.Name == "NX_Auth"      or old.Name == "NX_Confirm"
    or old.Name == "NX_StatusBar" or old.Name == "NX_RenewalPopup"
    or old.Name == "NX_RenewNotif" then
        old:Destroy()
    end
end
...
local LINKVERTISE_URL = "https://direct-link.net/5241712/CR6BlRSadnaC"
local SAVED_EMAIL_KEY   = "NX_SavedEmail"
local SAVED_SESSION_KEY = "NX_Session"

-- DEFAULT CREDENTIALS (pre-fill email/key here if you want auto-fill)
local DEFAULT_EMAIL = ""   -- e.g. "yourmail@gmail.com"
local DEFAULT_KEY   = ""   -- e.g. "NMX-XXXXXXXX-XXXXXXXX"

-- STATUS BAR CONFIG
local SB_CHECK_INTERVAL = 60    -- seconds between key validity checks
local SB_GRACE_PERIOD   = 600   -- 10 minutes grace period
...
local function getScale()
    ...
    if not cam then return 0.78 end
...
local function apiValidate(email, licenseKey)
    ...
    if not ok or not result then return false, "Server Offline", nil end
...
pcall(function() authPanel.Draggable = true end)
...
emailBox.Text = DEFAULT_EMAIL ~= "" and DEFAULT_EMAIL or loadSavedEmail()
...
cardStat(infoCard, "Display Name", tostring(lp.DisplayName or lp.Name))
...
-- ============================================================
-- ============================================================
-- STATUS BAR LIVE TRACKERS
-- ============================================================
local _sbFPS  = 60
local _sbPing = 0
local _sbFC, _sbFL = 0, tick()
...
-- Fallback FPS via Heartbeat for executors that block RenderStepped
pcall(function()
    if _sbFPS == 60 then -- still default, RenderStepped may not have fired
    ...
    while true do
        ...
        pcall(function()
            ...
            if pingItem and pingItem.GetValue then
                _sbPing = math.floor(pingItem:GetValue())
            end
        end)
    end
end)
...
-- ============================================================
local NP_Points   = 0
local NP_PerHour  = 250 -- bumps to 500 for Premium
local NP_Timer    = 0
local NPLabel     = nil -- set after shop page built

task.spawn(function()
    while true do
        task.wait(1)
        if sessionData.authenticated then
            NP_Timer += 1
            if NP_Timer >= 3600 then
                NP_Timer  = 0
                NP_Points += NP_PerHour
                if NPLabel and NPLabel.Parent then
                    NPLabel.Text = tostring(NP_Points) .. " NP"
                end
            end
        end
    end
...
-- ── INVENTORY PAGE ──
-- ============================================================
local invPage = pages["Inventory"]
pageHeader(invPage, "📦 Inventory")

local invStatCard = makeCard(invPage, "Inventory Stats")
cardStat(invStatCard, "Total Fruits",    "0")
cardStat(invStatCard, "Mythical Fruits", "0")
cardStat(invStatCard, "Legendary Fruits","0")
cardStat(invStatCard, "Inventory Value", "—")
cardStat(invStatCard, "Trading Value",   "—")

local invFeatCard = makeCard(invPage, "Inventory Features")
cardToggle(invFeatCard, "Inventory Scanner")
cardToggle(invFeatCard, "Fruit Inventory")
cardToggle(invFeatCard, "Item Inventory")
cardToggle(invFeatCard, "Inventory Worth Calculator")
cardToggle(invFeatCard, "Trading Value Calculator")
cardSpacer(invFeatCard, 2)
cardBtnRow(invFeatCard, {{text="🔍 Scan Now"},{text="📋 Export"}})

-- ============================================================
-- ── SECURITY PAGE ──
-- ============================================================
local secPage = pages["Security"]
pageHeader(secPage, "🔐 Security Center")

local secStatusCard = makeCard(secPage, "Account Security")
cardStat(secStatusCard, "Security Status",  "✔ Secure")
cardStat(secStatusCard, "Account Status",   "Active")
cardStat(secStatusCard, "Warning Counter",  "0 / 3")
cardStat(secStatusCard, "License Status",   "Active")
cardStat(secStatusCard, "Premium Status",   "Premium")
cardStat(secStatusCard, "Active Sessions",  "1")
cardSpacer(secStatusCard, 2)
cardBtn(secStatusCard, "📜 Login History")
cardBtn(secStatusCard, "🔐 Active Sessions")

local secWarnCard = makeCard(secPage, "⚠️ Warning System")
local warn1 = cardStat(secWarnCard, "Warning 1 / 3", "—")
local warn2 = cardStat(secWarnCard, "Warning 2 / 3", "—")
local warn3 = cardStat(secWarnCard, "Warning 3 / 3", "—")
local warnStatus = cardStat(secWarnCard, "Account Status", "✔ Good Standing")
cardSpacer(secWarnCard, 2)
cardBtn(secWarnCard, "📩 Submit Appeal Request")

-- ============================================================
-- ── ANNOUNCEMENTS PAGE ──
-- ============================================================
local annPage = pages["Announcements"]
pageHeader(annPage, "📢 Announcements")

local function makeAnnouncementItem(page, title, tag, content, date)
    local card = makeCard(page, "")
    local tagLbl = Instance.new("TextLabel", card)
    tagLbl.Text             = tag
    tagLbl.Font             = Enum.Font.GothamBold
    tagLbl.TextSize         = 7
    tagLbl.TextColor3       = ACCENT
    tagLbl.Size             = UDim2.new(0, 80, 0, 12)
    tagLbl.BackgroundTransparency = 1
    tagLbl.TextXAlignment   = Enum.TextXAlignment.Left
    tagLbl.ZIndex           = 6
    tagLbl.Parent           = card
    table.insert(themeTexts, tagLbl)
    ...
    contentLbl.TextColor3       = C.Gray
    contentLbl.Size             = UDim2.new(1,-14,0,28)
    ...
    dateLbl.TextSize         = 8
    dateLbl.TextColor3       = Color3.fromRGB(70, 65, 85)
    dateLbl.Size             = UDim2.new(1,-14,0,12)
...
makeAnnouncementItem(annPage, "Nemesis X v2.0 Released!", "🚀 UPDATE",
    "Major update! New authentication system, progression managers, and 70+ features added.", "Today")
makeAnnouncementItem(annPage, "Lucky Spin System Live", "🎰 EVENT",
    "Lucky Spin is now available. Win up to Lifetime Premium! Check Rewards tab.", "Today")
makeAnnouncementItem(annPage, "Sea Events Revamp", "🌊 PATCH",
    "Leviathan Manager completely rewritten with stage tracking and reward detection.", "Yesterday")
makeAnnouncementItem(annPage, "Premium Announcement", "💎 PREMIUM",
    "Premium users now earn 500 NP/hour. Shop updated with new cosmetic items.", "2 days ago")
makeAnnouncementItem(annPage, "System Maintenance", "⚙️ SYSTEM",
    "Brief maintenance scheduled. All systems nominal. Thank you for your patience.", "3 days ago")

-- ============================================================
-- ── LEADERBOARDS PAGE ──
-- ============================================================
local lbPage = pages["Leaderboards"]
pageHeader(lbPage, "🏆 Hall of Fame")

local function makeLeaderEntry(page, rank, username, title, date, isGold)
    local card = makeCard(page, "")
    local rankLbl = Instance.new("TextLabel", card)
    rankLbl.Text             = "#" .. rank
    rankLbl.Font             = Enum.Font.GothamBlack
    rankLbl.TextSize         = 14
    rankLbl.TextColor3       = isGold and C.Gold or C.Gray
    rankLbl.Size             = UDim2.new(0, 30, 0, 22)
    ...
    uLbl.TextSize         = 10
    uLbl.TextColor3       = isGold and C.Gold or C.White
    uLbl.Size             = UDim2.new(1,-14,0,16)
    ...
    tLbl.TextSize         = 9
    tLbl.TextColor3       = isGold and ACCENT or C.Gray
    tLbl.Size             = UDim2.new(1,-14,0,12)
    ...
    if date then
        ...
        dLbl.TextSize         = 8
        dLbl.TextColor3       = Color3.fromRGB(70,65,85)
        dLbl.Size             = UDim2.new(1,-14,0,11)
...
local lbHdr1 = makeCard(lbPage, "👑 Lifetime Premium Winners")
makeLeaderEntry(lbPage, 1, "CursedExility",   "Lifetime Premium Winner • Lucky Spin", "Season 1", true)
makeLeaderEntry(lbPage, 2, "—",               "Awaiting next winner",                 nil,        false)

local lbHdr2 = makeCard(lbPage, "🎰 Lucky Spin Champions")
makeLeaderEntry(lbPage, 1, "CursedExility",   "Lucky Spin Winner",                    "Season 1", true)
makeLeaderEntry(lbPage, 2, "—",               "Awaiting next winner",                 nil,        false)

local lbHdr3 = makeCard(lbPage, "🏅 Previous Season Winners")
makeLeaderEntry(lbPage, 1, "—", "Season 1 coming soon", nil, false)

-- ============================================================
-- ── REWARDS PAGE ──
-- ============================================================
local rewPage = pages["Rewards"]
pageHeader(rewPage, "🎁 Rewards")

-- Daily rewards
local dailyCard = makeCard(rewPage, "📅 Daily Rewards")
local dailyRewards = {
    {"Day 1", "250 NP"},
    {"Day 2", "500 NP"},
    {"Day 3", "750 NP + 🎰 Lucky Spin Ticket"},
    {"Day 4", "1000 NP"},
    {"Day 5", "1500 NP"},
    {"Day 6", "2000 NP + 🎰 Lucky Spin Ticket"},
    {"Day 7", "5000 NP + 1 Day Premium Trial"},
}
for _, v in ipairs(dailyRewards) do
    cardStat(dailyCard, v[1], v[2])
end
cardSpacer(dailyCard, 2)
cardBtn(dailyCard, "🎁 Claim Daily Reward")

-- Lucky Spin
local spinCard = makeCard(rewPage, "🎰 Lucky Spin")
cardStat(spinCard, "Tickets Available", "0")
cardStat(spinCard, "Last Spin", "Never")
cardStat(spinCard, "Lifetime Chance", "0.25%")
cardSpacer(spinCard, 2)
-- Spin wheel display
local spinDisplay = Instance.new("Frame", spinCard)
spinDisplay.Size             = UDim2.new(1,-14,0,22)
spinDisplay.BackgroundColor3 = C.Panel
spinDisplay.BorderSizePixel  = 0
spinDisplay.ZIndex           = 6
spinDisplay.Parent           = spinCard
Instance.new("UICorner", spinDisplay).CornerRadius = UDim.new(0,5)
local spinLbl = Instance.new("TextLabel", spinDisplay)
spinLbl.Text             = "🎰  Spin to win up to  Lifetime Premium!"
spinLbl.Font             = Enum.Font.GothamBold
spinLbl.TextSize         = 9
spinLbl.TextColor3       = C.Gold
spinLbl.Size             = UDim2.new(1,0,1,0)
spinLbl.BackgroundTransparency = 1
spinLbl.TextXAlignment   = Enum.TextXAlignment.Center
spinLbl.ZIndex           = 7
spinLbl.Parent           = spinDisplay
cardSpacer(spinCard, 2)
cardBtn(spinCard, "🎰 Use Spin Ticket")

-- Reward pool
local poolCard = makeCard(rewPage, "🎁 Reward Pool")
local pool = {
    "250 NP", "500 NP", "1,000 NP", "5,000 NP", "10,000 NP",
    "1 Day Premium", "3 Day Premium", "1 Week Premium",
    "1 Month Premium", "⭐ Lifetime Premium (0.25%)",
}
for _, v in ipairs(pool) do
    local f = Instance.new("Frame", poolCard)
    f.Size             = UDim2.new(1,-14,0,16)
    f.BackgroundTransparency = 1
    f.ZIndex           = 6
    f.Parent           = poolCard
    ...
    dot.BackgroundColor3 = v:find("Lifetime") and C.Gold or ACCENT
    dot.BorderSizePixel  = 0
    dot.ZIndex           = 7
    dot.Parent           = f
    Instance.new("UICorner", dot).CornerRadius = UDim.new(1,0)
    if v:find("Lifetime") then table.insert(themeFrames, dot) end
    local lbl = Instance.new("TextLabel", f)
    lbl.Text             = v
    lbl.Font             = v:find("Lifetime") and Enum.Font.GothamBold or Enum.Font.Gotham
    lbl.TextSize         = v:find("Lifetime") and 10 or 9
    lbl.TextColor3       = v:find("Lifetime") and C.Gold or C.Gray
...
-- Referral
local refCard = makeCard(rewPage, "🔗 Referral System")
cardStat(refCard, "Referrals", "0")
cardStat(refCard, "Referral Bonus", "500 NP each")
cardBtn(refCard, "📋 Copy Referral Link")

-- Reward history
local histCard = makeCard(rewPage, "📜 Reward History")
local histLbl = Instance.new("TextLabel", histCard)
histLbl.Text             = "No rewards claimed yet."
histLbl.Font             = Enum.Font.Gotham
histLbl.TextSize         = 9
histLbl.TextColor3       = C.Gray
histLbl.Size             = UDim2.new(1,-14,0,20)
histLbl.BackgroundTransparency = 1
histLbl.TextXAlignment   = Enum.TextXAlignment.Left
histLbl.ZIndex           = 6
histLbl.Parent           = histCard

-- ============================================================
-- ── SHOP PAGE ──
-- ============================================================
local shopPage = pages["Shop"]
pageHeader(shopPage, "🛒 Nemesis Shop")

-- NP Balance
local npCard = makeCard(shopPage, "💰 Nemesis Points")
NPLabel = cardStat(npCard, "Your Balance", "0 NP")
cardStat(npCard, "Earn Rate", "250 NP/hr (Free)")
cardStat(npCard, "Premium Rate", "500 NP/hr")
cardSpacer(npCard, 2)
cardBtn(npCard, "💰 Earn More NP")

-- Premium keys
local premKeysCard = makeCard(shopPage, "🔑 Premium Keys")
local premItems = {
    {"1 Day Premium",      "2,000 NP"},
    {"3 Day Premium",      "5,000 NP"},
    {"1 Week Premium",     "10,000 NP"},
    {"1 Month Premium",    "35,000 NP"},
    {"Lifetime Premium",   "100,000 NP"},
}
for _, v in ipairs(premItems) do
    local f = Instance.new("Frame", premKeysCard)
    f.Size             = UDim2.new(1,-14,0,22)
    f.BackgroundTransparency = 1
    f.ZIndex           = 6
    f.Parent           = premKeysCard
    ...
    pb.Size             = UDim2.new(0.4,-2,0,18)
    pb.Position         = UDim2.new(0.6,0,0.5,-9)
    ...
    pbs.Thickness=1; pbs.Color=ACCENT; pbs.Transparency=0.4
    table.insert(themeStrokes, pbs)
    table.insert(themeTexts, pb)
end

-- Cosmetics
local cosCard = makeCard(shopPage, "🎨 Cosmetics")
local cosItems = {
    "Premium Themes", "Profile Borders", "Chat Badges", "Premium Effects"
}
for _, v in ipairs(cosItems) do
    local f = Instance.new("Frame", cosCard)
    f.Size             = UDim2.new(1,-14,0,22)
    f.BackgroundTransparency = 1
    f.ZIndex           = 6
    f.Parent           = cosCard
    ...
    pb.Font             = Enum.Font.GothamBold
    pb.TextSize         = 8
    pb.TextColor3       = C.Gold
    pb.Size             = UDim2.new(0.3,-2,0,18)
    pb.Position         = UDim2.new(0.7,0,0.5,-9)
    pb.BackgroundColor3 = Color3.fromRGB(30,22,8)
    pb.BorderSizePixel  = 0
    pb.AutoButtonColor  = false
    pb.ZIndex           = 6
    pb.Parent           = f
...
-- ── LANGUAGE PAGE ──
-- ============================================================
local langPage = pages["Language"]
pageHeader(langPage, "🌍 Language Settings")

local langCard = makeCard(langPage, "Select Language")
cardStat(langCard, "Current Language", "English")
cardSpacer(langCard, 2)

-- Search bar
local searchWrapper = Instance.new("Frame", langCard)
searchWrapper.Size             = UDim2.new(1,-14,0,28)
searchWrapper.BackgroundColor3 = C.Input
searchWrapper.BorderSizePixel  = 0
searchWrapper.ZIndex           = 6
searchWrapper.Parent           = langCard
Instance.new("UICorner", searchWrapper).CornerRadius = UDim.new(0,6)
local searchStroke = Instance.new("UIStroke", searchWrapper)
searchStroke.Thickness=1; searchStroke.Color=ACCENT; searchStroke.Transparency=0.55
table.insert(themeStrokes, searchStroke)
...
searchBox.PlaceholderColor3 = Color3.fromRGB(80,70,95)
...
searchBox.Focused:Connect(function()
    TweenService:Create(searchStroke, TweenInfo.new(0.15), {Transparency=0.1}):Play()
end)
searchBox.FocusLost:Connect(function()
    TweenService:Create(searchStroke, TweenInfo.new(0.15), {Transparency=0.55}):Play()
end)

cardSpacer(langCard, 2)

-- Scrollable language list
local langScroll = Instance.new("ScrollingFrame", langCard)
langScroll.Size                = UDim2.new(1,-14,0,100)
langScroll.BackgroundColor3    = C.Card
langScroll.BorderSizePixel     = 0
langScroll.ScrollBarThickness  = 3
langScroll.ScrollBarImageColor3 = ACCENT
langScroll.CanvasSize          = UDim2.new(0,0,0,0)
langScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y
langScroll.ZIndex              = 6
langScroll.Parent              = langCard
Instance.new("UICorner", langScroll).CornerRadius = UDim.new(0,5)
local langLayout = Instance.new("UIListLayout", langScroll)
langLayout.Padding   = UDim.new(0,1)
langLayout.SortOrder = Enum.SortOrder.LayoutOrder

local LANGUAGES = {
    "English","Filipino","Spanish","French","German","Italian","Portuguese",
    "Russian","Japanese","Korean","Chinese (Simplified)","Chinese (Traditional)",
    "Arabic","Hindi","Indonesian","Malay","Turkish","Vietnamese","Thai",
    "Dutch","Polish","Swedish","Norwegian","Danish","Finnish","Czech",
    "Romanian","Hungarian","Greek","Hebrew","Ukrainian","Catalan","Croatian",
    "Slovak","Bulgarian","Serbian","Lithuanian","Latvian","Estonian",
    "Slovenian","Albanian","Macedonian","Bosnian","Icelandic","Welsh",
    "Irish","Basque","Galician","Maltese","Bengali","Urdu","Persian",
    "Swahili","Afrikaans","Zulu","Xhosa","Hausa","Amharic","Yoruba",
    "Tamil","Telugu","Malayalam","Kannada","Gujarati","Marathi","Punjabi",
    "Nepali","Sinhala","Burmese","Khmer","Lao","Mongolian",
}

local currentLangBtn = nil
for li, langName in ipairs(LANGUAGES) do
    local lb = Instance.new("TextButton", langScroll)
    lb.Size             = UDim2.new(1,0,0,20)
    lb.BackgroundColor3 = li == 1 and ACCENT:Lerp(C.BG,0.7) or C.Card
    lb.BackgroundTransparency = li == 1 and 0 or 1
    lb.Text             = langName
    lb.Font             = li == 1 and Enum.Font.GothamBold or Enum.Font.Gotham
    lb.TextSize         = 9
    lb.TextColor3       = li == 1 and ACCENT or C.Gray
    lb.LayoutOrder      = li
    lb.ZIndex           = 7
    lb.BorderSizePixel  = 0
    lb.AutoButtonColor  = false
    lb.Parent           = langScroll
    local lbpad = Instance.new("UIPadding",lb)
    lbpad.PaddingLeft = UDim.new(0,8)
    if li == 1 then
        table.insert(themeTexts, lb)
        currentLangBtn = lb
    end
    lb.MouseButton1Click:Connect(function()
        if currentLangBtn then
            currentLangBtn.BackgroundTransparency = 1
            currentLangBtn.Font                   = Enum.Font.Gotham
            currentLangBtn.TextColor3             = C.Gray
        end
...
searchBox:GetPropertyChangedSignal("Text"):Connect(function()
    ...
    for _, child in ipairs(langScroll:GetChildren()) do
...
-- ============================================================
-- PREMIUM LOCK SYSTEM
-- Locked features display in dedicated card in Settings
-- ============================================================
local premLockCard = makeCard(pages["Settings"], "🔒 Premium Features")
local premLockedItems = {
    "Nemesis AI", "Custom UI", "Custom Floating Action Button",
    "Fruit Hunter Pro", "Live Events Tracker", "Premium Themes", "Auto Execute",
}
for _, v in ipairs(premLockedItems) do
    local f = Instance.new("Frame", premLockCard)
    f.Size             = UDim2.new(1,-14,0,22)
    f.BackgroundTransparency = 1
    f.ZIndex           = 6
    f.Parent           = premLockCard
    local nl = Instance.new("TextLabel", f)
    nl.Text             = "🔒 " .. v
    nl.Font             = Enum.Font.Gotham
    nl.TextSize         = 10
    ...
    nl.ZIndex           = 6
    nl.Parent           = f
    local ub = Instance.new("TextButton", f)
    ub.Text             = "⬆ Upgrade"
    ub.Font             = Enum.Font.GothamBold
    ub.TextSize         = 8
    ub.TextColor3       = ACCENT
    ub.Size             = UDim2.new(0.35,-2,0,18)
    ub.Position         = UDim2.new(0.65,0,0.5,-9)
    ub.BackgroundColor3 = C.Panel
    ub.BorderSizePixel  = 0
    ub.AutoButtonColor  = false
    ub.ZIndex           = 6
    ub.Parent           = f
    Instance.new("UICorner", ub).CornerRadius = UDim.new(0,4)
    local ubs = Instance.new("UIStroke", ub)
    ubs.Thickness=1; ubs.Color=ACCENT; ubs.Transparency=0.5
    table.insert(themeStrokes, ubs)
    table.insert(themeTexts, ub)
end
...
local function openMainUI()
    ...
    -- Start status bar session
    getgenv().NX_Authenticated = true
    getgenv().NX_Key           = sessionData.licenseKey
    if getgenv().NX_StartSession then
        getgenv().NX_StartSession()
    end
...
-- INIT
-- ============================================================
print("[NX Premium v3] Auth UI Ready | " .. detectExecutor() .. " | +7 New Tabs | Lucky Spin | Shop | Security")

-- ============================================================
-- ── TOP STATUS BAR (Void Hub / Sacred Style) ──
-- ============================================================

local BarSG = Instance.new("ScreenGui")
BarSG.Name           = "NX_StatusBar"
BarSG.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
BarSG.ResetOnSpawn   = false
BarSG.IgnoreGuiInset = true
BarSG.DisplayOrder   = 998
BarSG.Parent         = playerGui

-- Pill container — 280x27px, top center, 8px from top
local pill = Instance.new("Frame", BarSG)
pill.Name             = "Pill"
pill.Size             = UDim2.new(0, 280, 0, 27)
pill.Position         = UDim2.new(0.5, -140, 0, 8)
pill.BackgroundColor3 = Color3.fromRGB(10, 6, 12)
...
pillStroke.Color        = Color3.fromRGB(200, 30, 50)
pillStroke.Transparency = 0.3

local pillGrad = Instance.new("UIGradient", pill)
pillGrad.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0,   Color3.fromRGB(18, 8,  14)),
    ColorSequenceKeypoint.new(0.5, Color3.fromRGB(22, 10, 16)),
    ColorSequenceKeypoint.new(1,   Color3.fromRGB(18, 8,  14)),
})
...
task.spawn(function()
    ...
    while pill and pill.Parent do
        ...
        if pillStroke and pillStroke.Parent then
            pillStroke.Transparency = 0.15 + 0.25 * math.abs(math.sin(t * 0.6))
        end
    end
end)

-- Horizontal layout
local sbLayout = Instance.new("UIListLayout", pill)
...
local sbPad = Instance.new("UIPadding", pill)
sbPad.PaddingLeft  = UDim.new(0, 8)
sbPad.PaddingRight = UDim.new(0, 8)

-- Separator helper
local function sbSep(lo)
    local f = Instance.new("Frame", pill)
    f.Size             = UDim2.new(0, 1, 0.55, 0)
    f.BackgroundColor3 = Color3.fromRGB(80, 40, 50)
...
-- Label section helper
local function sbLbl(text, width, color, id, lo)
    ...
    lbl.TextSize         = 11
    lbl.TextColor3       = color or Color3.fromRGB(220, 220, 230)
    lbl.TextXAlignment   = Enum.TextXAlignment.Center
    lbl.ZIndex           = 11
    lbl.LayoutOrder      = lo or 1
    return lbl
end

-- Status dot
local sbDotWrap = Instance.new("Frame", pill)
sbDotWrap.Size             = UDim2.new(0, 20, 1, 0)
sbDotWrap.BackgroundTransparency = 1
sbDotWrap.ZIndex           = 11
sbDotWrap.LayoutOrder      = 0

local sbDot = Instance.new("Frame", sbDotWrap)
sbDot.Size             = UDim2.new(0, 8, 0, 8)
sbDot.Position         = UDim2.new(0.5, -4, 0.5, -4)
sbDot.BackgroundColor3 = Color3.fromRGB(45, 210, 110)
sbDot.BorderSizePixel  = 0
sbDot.ZIndex           = 12
Instance.new("UICorner", sbDot).CornerRadius = UDim.new(1, 0)

local sbDotGlow = Instance.new("ImageLabel", sbDotWrap)
sbDotGlow.Size             = UDim2.new(0, 18, 0, 18)
sbDotGlow.Position         = UDim2.new(0.5, -9, 0.5, -9)
sbDotGlow.BackgroundTransparency = 1
sbDotGlow.Image            = "rbxassetid://5779559284"
sbDotGlow.ImageColor3      = Color3.fromRGB(45, 210, 110)
sbDotGlow.ImageTransparency = 0.5
sbDotGlow.ZIndex           = 11

local sbNameLbl = sbLbl("Nemesis X", 72,  Color3.fromRGB(210, 60, 75), "SBName",    1)
sbNameLbl.Font    = Enum.Font.GothamBlack
sbNameLbl.TextSize = 10
sbSep(2)
local sbSessionLbl = sbLbl("0h 00m",  54,  Color3.fromRGB(220,220,230), "SBSession", 3)
sbSep(4)
local sbPingLbl    = sbLbl("--ms",    44,  Color3.fromRGB(200,200,210), "SBPing",    5)
sbSep(6)
local sbFPSLbl     = sbLbl("-- FPS",  48,  Color3.fromRGB(200,200,210), "SBFPS",     7)

-- Dot pulse animation
local function sbStartPulse(isValid)
    local baseColor = isValid and Color3.fromRGB(45,210,110) or Color3.fromRGB(215,60,60)
    local duration  = isValid and 1.2 or 0.9
    sbDot.BackgroundColor3  = baseColor
    sbDotGlow.ImageColor3   = baseColor
    task.spawn(function()
        while sbDot and sbDot.Parent do
            TweenService:Create(sbDot, TweenInfo.new(duration/2, Enum.EasingStyle.Sine), {
                Size = UDim2.new(0,10,0,10), Position = UDim2.new(0.5,-5,0.5,-5)
            }):Play()
            TweenService:Create(sbDotGlow, TweenInfo.new(duration/2, Enum.EasingStyle.Sine), {
                ImageTransparency = 0.2
            }):Play()
            task.wait(duration/2)
            TweenService:Create(sbDot, TweenInfo.new(duration/2, Enum.EasingStyle.Sine), {
                Size = UDim2.new(0,7,0,7), Position = UDim2.new(0.5,-3.5,0.5,-3.5)
            }):Play()
            TweenService:Create(sbDotGlow, TweenInfo.new(duration/2, Enum.EasingStyle.Sine), {
...
task.spawn(function()
    while true do
        task.wait(0.5)
        pcall(function()
            if sbFPSLbl and sbFPSLbl.Parent then
                sbFPSLbl.Text      = tostring(_sbFPS) .. " FPS"
                sbFPSLbl.TextColor3 = _sbFPS >= 50 and Color3.fromRGB(45,210,110)
                    or _sbFPS >= 30 and Color3.fromRGB(240,185,55)
                    or Color3.fromRGB(215,60,60)
            end
            if sbPingLbl and sbPingLbl.Parent then
                sbPingLbl.Text      = tostring(_sbPing) .. "ms"
                sbPingLbl.TextColor3 = _sbPing < 100 and Color3.fromRGB(45,210,110)
                    or _sbPing < 250 and Color3.fromRGB(240,185,55)
                    or Color3.fromRGB(215,60,60)
            end
        end)
    end
...
-- Session timer (starts after auth)
local _sbSessionStart = nil
local function sbStartSession()
    _sbSessionStart = tick()
    task.spawn(function()
        while _sbSessionStart do
            task.wait(1)
            local e    = math.floor(tick() - _sbSessionStart)
            local h    = math.floor(e / 3600)
            local m    = math.floor((e % 3600) / 60)
            if sbSessionLbl and sbSessionLbl.Parent then
                sbSessionLbl.Text = h .. "h " .. string.format("%02dm", m)
            end
        end
    end)
end
...
PopupSG.Name           = "NX_RenewalPopup"
PopupSG.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
PopupSG.ResetOnSpawn   = false
PopupSG.IgnoreGuiInset = true
PopupSG.DisplayOrder   = 999
PopupSG.Enabled        = false
PopupSG.Parent         = playerGui

local popBG = Instance.new("Frame", PopupSG)
popBG.Size                = UDim2.new(1,0,1,0)
popBG.BackgroundColor3    = Color3.new(0,0,0)
popBG.BackgroundTransparency = 0.55
popBG.BorderSizePixel     = 0
popBG.ZIndex              = 1

local popup = Instance.new("Frame", PopupSG)
popup.Size             = UDim2.new(0,320,0,280)
popup.Position         = UDim2.new(0.5,-160,0.5,-140)
popup.BackgroundColor3 = Color3.fromRGB(10,6,12)
...
popStroke.Thickness = 1.5; popStroke.Color = Color3.fromRGB(215,60,60); popStroke.Transparency = 0.1
...
task.spawn(function()
    ...
    while popup and popup.Parent do
        ...
        if popStroke and popStroke.Parent then
            popStroke.Transparency = 0.05 + 0.35 * math.abs(math.sin(t*0.7))
        end
    end
end)

local popTopBar = Instance.new("Frame", popup)
popTopBar.Size=UDim2.new(1,0,0,4); popTopBar.BackgroundColor3=Color3.fromRGB(215,60,60)
popTopBar.BorderSizePixel=0; popTopBar.ZIndex=3
Instance.new("UICorner",popTopBar).CornerRadius=UDim.new(0,12)
local ptFix=Instance.new("Frame",popTopBar)
ptFix.Size=UDim2.new(1,0,0.5,0); ptFix.Position=UDim2.new(0,0,0.5,0)
ptFix.BackgroundColor3=Color3.fromRGB(215,60,60); ptFix.BorderSizePixel=0

local function popLbl(text,y,size,color,bold,wrap)
    local l=Instance.new("TextLabel",popup)
    l.Text=text; l.Font=bold and Enum.Font.GothamBlack or Enum.Font.Gotham
    l.TextSize=size; l.TextColor3=color
    l.Size=UDim2.new(1,-20,0,(bold and (size+8) or (size+6)))
    l.Position=UDim2.new(0,10,0,y)
    l.BackgroundTransparency=1; l.TextXAlignment=Enum.TextXAlignment.Center
    l.TextWrapped=wrap or false; l.ZIndex=3
    return l
end

popLbl("⚠",14,28,Color3.fromRGB(215,60,60),true)
popLbl("Key Renewal Required",54,14,Color3.fromRGB(230,230,240),true)
popLbl("Your Nemesis X license has expired.\nYou have 10 minutes to renew your key\nbefore all features are disabled.",
    78,10,Color3.fromRGB(140,130,150),false,true)

local popCountdown = popLbl("09:59",126,22,Color3.fromRGB(215,60,60),true)

-- Key input
local popInputWrap = Instance.new("Frame",popup)
popInputWrap.Size=UDim2.new(0,280,0,32); popInputWrap.Position=UDim2.new(0.5,-140,0,162)
popInputWrap.BackgroundColor3=Color3.fromRGB(18,12,22); popInputWrap.BorderSizePixel=0; popInputWrap.ZIndex=3
Instance.new("UICorner",popInputWrap).CornerRadius=UDim.new(0,8)
local piS=Instance.new("UIStroke",popInputWrap)
piS.Thickness=1; piS.Color=Color3.fromRGB(80,40,60); piS.Transparency=0.3

local popKeyBox = Instance.new("TextBox",popInputWrap)
popKeyBox.Size=UDim2.new(1,-16,1,0); popKeyBox.Position=UDim2.new(0,8,0,0)
popKeyBox.BackgroundTransparency=1; popKeyBox.Text=""
popKeyBox.PlaceholderText="Enter your license key..."
popKeyBox.PlaceholderColor3=Color3.fromRGB(80,70,90)
popKeyBox.Font=Enum.Font.Gotham; popKeyBox.TextSize=11
popKeyBox.TextColor3=Color3.fromRGB(220,220,230)
popKeyBox.TextXAlignment=Enum.TextXAlignment.Left
popKeyBox.ClearTextOnFocus=false; popKeyBox.ZIndex=4
popKeyBox.Focused:Connect(function() piS.Transparency=0; piS.Color=Color3.fromRGB(215,60,60) end)
popKeyBox.FocusLost:Connect(function() piS.Transparency=0.3; piS.Color=Color3.fromRGB(80,40,60) end)

-- Popup buttons
local function popBtn(text,y,bg,tc)
    local b=Instance.new("TextButton",popup)
    b.Size=UDim2.new(0,280,0,30); b.Position=UDim2.new(0.5,-140,0,y)
    b.BackgroundColor3=bg; b.Text=text; b.Font=Enum.Font.GothamBold
    b.TextSize=11; b.TextColor3=tc; b.ZIndex=3; b.BorderSizePixel=0; b.AutoButtonColor=false
    Instance.new("UICorner",b).CornerRadius=UDim.new(0,7)
    return b
end

local popActivateBtn = popBtn("⚡ Activate Key",202,Color3.fromRGB(180,30,45),Color3.new(1,1,1))
local popGetKeyBtn   = popBtn("🔗 Get Key",238,Color3.fromRGB(20,14,26),Color3.fromRGB(215,60,60))

local popCloseBtn = Instance.new("TextButton",popup)
popCloseBtn.Size=UDim2.new(0,22,0,22); popCloseBtn.Position=UDim2.new(1,-28,0,8)
popCloseBtn.BackgroundColor3=Color3.fromRGB(20,14,26); popCloseBtn.Text="✕"
popCloseBtn.Font=Enum.Font.GothamBold; popCloseBtn.TextSize=11
popCloseBtn.TextColor3=Color3.fromRGB(100,90,110); popCloseBtn.ZIndex=4
popCloseBtn.BorderSizePixel=0; popCloseBtn.AutoButtonColor=false
Instance.new("UICorner",popCloseBtn).CornerRadius=UDim.new(0,6)
popCloseBtn.MouseButton1Click:Connect(function()
    PopupSG.Enabled = false
end)

-- Key expiration state
local sbKeyValid      = true
local sbGraceActive   = false
local sbGraceThread   = nil
local sbFeatEnabled   = true
getgenv().NX_FeaturesEnabled = true
getgenv().NX_KeyValid        = true

local function sbDisableFeatures()
    sbFeatEnabled                = false
    getgenv().NX_FeaturesEnabled = false
    getgenv().NX_KeyValid        = false
end

local function sbEnableFeatures()
    sbFeatEnabled                = true
    getgenv().NX_FeaturesEnabled = true
    getgenv().NX_KeyValid        = true
end

local function sbOnExpired()
    sbKeyValid = false
    sbStartPulse(false)
    sbSessionLbl.Text      = "Renewal Required"
    sbSessionLbl.TextColor3 = Color3.fromRGB(215,60,60)
    PopupSG.Enabled        = true
    sbGraceActive          = true
    local remaining        = SB_GRACE_PERIOD
    if sbGraceThread then task.cancel(sbGraceThread) end
    sbGraceThread = task.spawn(function()
        while remaining > 0 do
            task.wait(1); remaining -= 1
            local m = math.floor(remaining/60)
            local s = remaining%60
            if popCountdown and popCountdown.Parent then
                popCountdown.Text = string.format("%02d:%02d",m,s)
            end
            if sbSessionLbl and sbSessionLbl.Parent then
                sbSessionLbl.Text = string.format("Renewal %02d:%02d",m,s)
            end
        end
        sbGraceActive = false
        sbDisableFeatures()
        if popCountdown and popCountdown.Parent then
            popCountdown.Text = "00:00"
        end
    end)
end

local function sbOnRestored()
    sbKeyValid    = true
    sbGraceActive = false
    if sbGraceThread then task.cancel(sbGraceThread); sbGraceThread=nil end
    sbEnableFeatures()
    sbStartPulse(true)
    sbDot.BackgroundColor3  = Color3.fromRGB(45,210,110)
    sbDotGlow.ImageColor3   = Color3.fromRGB(45,210,110)
    sbSessionLbl.TextColor3 = Color3.fromRGB(220,220,230)
    PopupSG.Enabled         = false

    -- Success notification
    local ns = Instance.new("ScreenGui")
    ns.Name="NX_RenewNotif"; ns.ResetOnSpawn=false
    ns.IgnoreGuiInset=true; ns.DisplayOrder=1000; ns.Parent=playerGui
    local nf = Instance.new("Frame",ns)
    nf.Size=UDim2.new(0,260,0,64); nf.Position=UDim2.new(0.5,-130,0,-70)
    nf.BackgroundColor3=Color3.fromRGB(10,6,12); nf.BorderSizePixel=0; nf.ZIndex=2
    Instance.new("UICorner",nf).CornerRadius=UDim.new(0,10)
    local nfs=Instance.new("UIStroke",nf)
    nfs.Thickness=1.2; nfs.Color=Color3.fromRGB(45,210,110); nfs.Transparency=0.2
    local nt=Instance.new("TextLabel",nf); nt.Text="✅ Key Activated"
    nt.Font=Enum.Font.GothamBlack; nt.TextSize=12; nt.TextColor3=Color3.fromRGB(45,210,110)
    nt.Size=UDim2.new(1,0,0,22); nt.Position=UDim2.new(0,0,0,8)
    nt.BackgroundTransparency=1; nt.TextXAlignment=Enum.TextXAlignment.Center; nt.ZIndex=3
    local nd=Instance.new("TextLabel",nf); nd.Text="Welcome Back! All Features Restored."
    nd.Font=Enum.Font.Gotham; nd.TextSize=9; nd.TextColor3=Color3.fromRGB(140,140,150)
    nd.Size=UDim2.new(1,-16,0,28); nd.Position=UDim2.new(0,8,0,30)
    nd.BackgroundTransparency=1; nd.TextXAlignment=Enum.TextXAlignment.Center
    nd.TextWrapped=true; nd.ZIndex=3
    TweenService:Create(nf,TweenInfo.new(0.3,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{
        Position=UDim2.new(0.5,-130,0,46)
    }):Play()
    task.delay(3,function()
        ...
        task.wait(0.35); ns:Destroy()
    end)
end

-- Activate key button
popActivateBtn.MouseButton1Click:Connect(function()
    local key = popKeyBox.Text:match("^%s*(.-)%s*$")
    if key == "" then
        popActivateBtn.Text="Enter a key first!"
        popActivateBtn.TextColor3=Color3.fromRGB(215,60,60)
        task.delay(1.5,function()
            popActivateBtn.Text="⚡ Activate Key"
            popActivateBtn.TextColor3=Color3.new(1,1,1)
        end)
        return
    end
    popActivateBtn.Text="Validating..."
    popActivateBtn.TextColor3=Color3.fromRGB(240,185,55)
    task.spawn(function()
        local ok,result = pcall(function()
            return HttpService:RequestAsync({
                Url    = REPLIT_URL.."/validate",
        ...
        if ok and result and result.StatusCode == 200 then
            ...
            local msg = (ok and result and result.StatusCode == 403) and "Expired / Bound"
                or (ok and result and result.StatusCode == 404) and "Invalid Key"
                or "Server Offline"
            popActivateBtn.Text=msg
            popActivateBtn.TextColor3=Color3.fromRGB(215,60,60)
...
-- Get key button
popGetKeyBtn.MouseButton1Click:Connect(function()
    pcall(function() setclipboard(LINKVERTISE_URL) end)
    popGetKeyBtn.Text="🔗 Link Copied!"
    task.delay(2,function() popGetKeyBtn.Text="🔗 Get Key" end)
end)

-- Periodic key expiry check
task.spawn(function()
    while true do
        task.wait(SB_CHECK_INTERVAL)
        if not sbKeyValid then continue end
        local key = tostring(getgenv().NX_Key or "")
...
-- Public session start (called after auth)
getgenv().NX_StartSession = function()
    sbStartSession()
    sbKeyValid = true
    sbStartPulse(true)
end

getgenv().NX_OnExpired  = sbOnExpired
getgenv().NX_OnRestored = sbOnRestored

print("[NX Combined] ✔ Loaded | Status Bar + Auth + 22 Tabs | by CursedExility")
