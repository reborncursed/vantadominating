local function AuroraCancelTween()
  pcall(function()
    if _G.TweenCache then _G.TweenCache:Cancel() end
    if _G.__AuroraActiveTween then _G.__AuroraActiveTween:Cancel() end
  end)
  _G.TweenCache = nil
  _G.__AuroraActiveTween = nil
end

local function AuroraStopVelocity(part)
...
task.spawn(function()
  local watched = {
    "AutoFarmNear","AutoFarmChest","AutoFarm_Bone","AutoFarmAllBoss",
    "AutoFarmIsland","AutoFarmDungeon","AutoHop_Dough","MasterAutoLevel",
    "AutoKillMob","AutoFarmRaid","FarmBoss","AutoFarmCandy","Level",
  }
  while task.wait(0.5) do
    local active = false
    for _, k in ipairs(watched) do
      if _G[k] then active = true break end
    end
    _G.__AuroraFarmActive = active
  end
end)
do
  ply = game.Players
...
if World1 then BossList = {"The Gorilla King","Chef","The Saw","Yeti","Mob Leader","Vice Admiral","Saber Expert","Warden","Chief Warden","Swan","Magma Admiral","Fishman Lord","Wysper","Thunder God","Cyborg","Ice Admiral","Greybeard"}
elseif World2 then BossList = {"Diamond","Jeremy","Fajita","Don Swan","Smoke Admiral","Ice Admiral","Tide Keeper","Darkbeard","Cursed Captain","Order"}
elseif World3 then BossList = {"Stone","Island Empress","Imperial Captain","Kilo Admiral","Captain Elephant","Beautiful Pirate","Longma","Tide Keeper","Cake Queen","Cake Prince","Dough King","rip_indra"}
end
...
local Human_v3_Mob = {"Orbitus","Jeremy","Diamond"}
...
Attack.Pos = function(model, dist)
  ...
  -- Solución para el error tipográfico de 'mode' a 'model' y soporte para CFrame/Vector3
  local pos = typeof(model) == "CFrame" and model.Position or (typeof(model) == "Vector3" and model or (model.Position and model.Position or model:GetPivot().Position))
...
Attack.DistH = function(model, dist)
  if not model or not model:FindFirstChild("HumanoidRootPart") then return true end
  return (Root.Position - model:FindFirstChild("HumanoidRootPart").Position).Magnitude > dist
end

Attack.Kill = function(model, Succes)
  if model and Succes and Attack.Alive(model) then
    ...
    if ToolTip == "Blox Fruit" then _tp(hrp.CFrame * CFrame.new(0,10,0) * CFrame.Angles(0,math.rad(90),0)) else _tp(hrp.CFrame * CFrame.new(0,30,0) * CFrame.Angles(0,math.rad(180),0))end
    if RandomCFrame then wait(.5)_tp(hrp.CFrame * CFrame.new(0, 30, 25)) wait(.5)_tp(hrp.CFrame * CFrame.new(25, 30, 0)) wait(.5)_tp(hrp.CFrame * CFrame.new(-25, 30 ,0)) wait(.5)_tp(hrp.CFrame * CFrame.new(0, 30, 25)) wait(.5)_tp(hrp.CFrame * CFrame.new(-25, 30, 0))end
  end
end
...
Attack.Kill2 = function(model, Succes)
  if model and Succes and Attack.Alive(model) then
    ...
    if not Equipped then return end
    local ToolTip = Equipped.ToolTip
    if ToolTip == "Blox Fruit" then _tp(hrp.CFrame * CFrame.new(0,10,0) * CFrame.Angles(0,math.rad(90),0)) else _tp(hrp.CFrame * CFrame.new(0,30,8) * CFrame.Angles(0,math.rad(180),0))end
    if RandomCFrame then wait(0.1)_tp(hrp.CFrame * CFrame.new(0, 30, 25)) wait(0.1)_tp(hrp.CFrame * CFrame.new(25, 30, 0)) wait(0.1)_tp(hrp.CFrame * CFrame.new(-25, 30 ,0)) wait(0.1)_tp(hrp.CFrame * CFrame.new(0, 30, 25)) wait(0.1)_tp(hrp.CFrame * CFrame.new(-25, 30, 0))end
  end
end
...
Attack.KillSea = function(model, Succes)
  if model and Succes then
    ...
    if not Equipped then return end
    local ToolTip = Equipped.ToolTip
    if ToolTip == "Blox Fruit" then _tp(hrp.CFrame * CFrame.new(0,10,0) * CFrame.Angles(0,math.rad(90),0)) else notween(hrp.CFrame * CFrame.new(0,50,8)) wait(.85)notween(hrp.CFrame * CFrame.new(0,400,0)) wait(1)end
  end
end
    ...
    if tostring(method) == "FireServer" then
      if tostring(args[1]) == "RemoteEvent" then
        if tostring(args[2]) ~= "true" and tostring(args[2]) ~= "false" then
          if MousePos and ((_G.FarmMastery_G and not SoulGuitar) or (_G.FarmMastery_Dev) or (_G.FarmBlazeEM) or (_G.Prehis_Skills) or (_G.SeaBeast1 or _G.FishBoat or _G.PGB or _G.Leviathan1 or (_G.Complete_Trials) or (_G.AimMethod and ABmethod == "Aim Player") or (_G.AimMethod and ABmethod == "Nearest Aim") or (_G.AimbotP or _G.AimbotN))) then
            args[2] = MousePos
  ...
  for _, v1 in pairs(workspace:GetChildren()) do
    if string.find(v1.Name, "Fruit") then
      ...
      if handle then
        ...
        handle.CFrame = CFrame.new(hrpPos.X, hrpPos.Y - 2, hrpPos.Z)
      end
    end
  end
end
...
Hop = function()
  -- Aurora patch: no hopear si hay farms activos (evita el "de la nada me fui a otra isla")
  if _G.__AuroraFarmActive and not _G.__AuroraAllowHop then
    return
  end
  pcall(function()
  ...
  while task.wait() do
    pcall(function()
      ...
      if getgenv().OnFarm then
        if block and block.Parent == workspace and b then
          local gap = (b.Position - block.Position).Magnitude
          if gap <= 140 then
            AuroraStopVelocity(b)
        ...
        AuroraCancelTween()
        if block and block.Parent == workspace and b then block.CFrame = b.CFrame end
        if c then
          for _, e in pairs(c:GetChildren()) do
...
_tp = function(target)
    ...
    local allowEntranceMove = _G.__AuroraAllowEntrance or _G.Teleport or _G.TPNpc or _G.TpPly or _G.TravelDres or _G.AutoZou or _G.TPDoor or _G.FindMirage or _G.TPGEAR or _G.TPFloor1 or _G.TPFloor2 or _G.TPFloor3 or _G.TPFloor4

    -- No usar portales/entrances automáticos mientras se farmea normal.
    -- Esos requestEntrance eran una causa común de aparecer en otra isla sin tocar hop server.
    if allowEntranceMove then
        pcall(function()
    ...
    AuroraCancelTween()
    AuroraStopVelocity(rootPart)
    if block and block.Parent == workspace then
        block.CFrame = rootPart.CFrame
    end

    local speed = tonumber(Settings and Settings["Tween Speed"]) or 300
    speed = math.clamp(speed, 220, 350)
    local tweenInfo = TweenInfo.new(math.max(distance / speed, 0.05), Enum.EasingStyle.Linear)
    local tween = game:GetService("TweenService"):Create(block, tweenInfo, {CFrame = gg})    
    _G.TweenCache = tween
    _G.__AuroraActiveTween = tween
    ...
    task.spawn(function() 
        while tween.PlaybackState == Enum.PlaybackState.Playing do 
            if not shouldTween then 
                tween:Cancel() 
                if _G.__AuroraActiveTween == tween then _G.__AuroraActiveTween = nil end
                if _G.TweenCache == tween then _G.TweenCache = nil end
                break 
            end 
            task.wait(0.1) 
        end 
...
old_tp = function(p) 
    ...
    if char and char:FindFirstChild("HumanoidRootPart") then
        AuroraCancelTween()
        AuroraStopVelocity(char.HumanoidRootPart)
  ...
  while task.wait() do
    pcall(function()
      if _G.SailBoat_Hydra or _G.WardenBoss or _G.AutoFactory or _G.HighestMirage or _G.HCM or _G.PGB or _G.Leviathan1 or _G.UPGDrago or _G.Complete_Trials or _G.TpDrago_Prehis or _G.BuyDrago or _G.AutoFireFlowers or _G.DT_Uzoth or _G.AutoBerry or _G.Prefully or _G.Prehis_Find or _G.Prehis_Skills or _G.Prehis_DB or _G.Prehis_DE or _G.FarmBlazeEM or _G.Dojoo or _G.CollectPresent or _G.AutoLawKak or _G.TpLab or _G.AutoPhoenixF or _G.AutoFarmChest or _G.AutoHytHallow or _G.LongsWord or _G.BlackSpikey or _G.AutoHolyTorch or _G.TrainDrago  or _G.AutoSaber or _G.FarmMastery_Dev or _G.CitizenQuest or _G.AutoEctoplasm or _G.KeysRen or _G.Auto_Rainbow_Haki or _G.obsFarm or _G.AutoBigmom or _G.Doughv2 or _G.AuraBoss or _G.Raiding or _G.Auto_Cavender or _G.TpPly or _G.Bartilo_Quest or _G.Level or _G.FarmEliteHunt or _G.AutoZou or _G.AutoFarm_Bone or getgenv().AutoMaterial or _G.CraftVM or _G.FrozenTP or _G.TPDoor or _G.AcientOne or _G.AutoFarmNear or _G.AutoRaidCastle or _G.DarkBladev3 or _G.AutoFarmRaid or _G.Auto_Cake_Prince or _G.Addealer or _G.TPNpc or _G.TwinHook or _G.FindMirage or _G.FarmChestM or _G.Shark or _G.TerrorShark or _G.Piranha or _G.MobCrew or _G.SeaBeast1 or _G.FishBoat or _G.AutoPole or _G.AutoPoleV2 or _G.Auto_SuperHuman or _G.AutoDeathStep or _G.Auto_SharkMan_Karate or _G.Auto_Electric_Claw or _G.AutoDragonTalon or _G.Auto_Def_DarkCoat or _G.Auto_God_Human or _G.Auto_Tushita or _G.AutoMatSoul or _G.AutoKenVTWO or _G.AutoSerpentBow or _G.AutoFMon or _G.Auto_Soul_Guitar or _G.TPGEAR or _G.AutoSaw or _G.AutoTridentW2 or _G.AutoEvoRace or _G.AutoGetQuestBounty or _G.MarinesCoat or _G.TravelDres or _G.Defeating or _G.DummyMan or _G.Auto_Yama or _G.Auto_SwanGG or _G.SwanCoat or _G.AutoEcBoss or _G.Auto_Mink or _G.Auto_Human or _G.Auto_Skypiea or _G.Auto_Fish or _G.CDK_TS or _G.CDK_YM or _G.CDK or _G.AutoFarmGodChalice or _G.AutoFistDarkness or _G.AutoMiror or _G.Teleport or _G.AutoKilo or _G.AutoGetUsoap or _G.Praying or _G.TryLucky or _G.AutoColShad or _G.AutoUnHaki or _G.Auto_DonAcces or _G.AutoRipIngay or _G.DragoV3 or _G.DragoV1 or _G.SailBoats or NextIs or _G.FarmGodChalice or _G.IceBossRen or senth or senth2 or _G.Lvthan or _G.beasthunter or _G.DangerLV or _G.Relic123 or _G.tweenKitsune or _G.Collect_Ember or _G.AutofindKitIs or _G.snaguine or _G.TwFruits or _G.tweenKitShrine or _G.Tp_LgS or _G.Tp_MasterA or _G.tweenShrine or _G.FarmMastery_G or _G.FarmMastery_S or _G.FarmBoss or _G.AutoFarmAllBoss or _G.AutoFishSlap or _G.FarmTyrant or _G.FarmPhaBinh or _G.AutoSpawnCP or _G.AutoBerryH or _G.AutoChestBP or _G.FarmEliteHop or _G.AutoHop_Dough or _G.AutoDoughKing or _G.AutoAttackDoughKing or _G.AutoChipFruit or _G.AutoChipBeli or _G.StartEvent or _G.AutoMysticIsland or _G.AutoPlayerHunter or _G.SafeMode or _G.AutoKillMob or _G.AutoStartPrehistoric or _G.AutoUnHaki or _G.AutoAttackRipIndra or _G.AutoFarmIsland or _G.AutoFarmDungeon or _G.AutoFarmCandy or _G.AutoTP_Gift or _G.AutoTPGift or _G.AutoTPAndCollect or _G.MasterAutoLevel or _G.MasterAutoCandy or _G.TPFloor1 or _G.TPFloor2 or _G.TPFloor3 or _G.TPFloor4 then
        ...
        AuroraStopVelocity(plr.Character.HumanoidRootPart)
...
GetQuests = function()
    ...
    -- Aurora: nivel máximo actual del juego
    local MAX_LEVEL = 2800

    -- Aurora fix: acota los quests al mar actual para que no elija
    -- una quest del mar 1 estando en mar 2/3 (causaba el "se va al
    -- centro del mar" porque la CFrame de la isla no existe en este mundo).
    local minLv, maxLv
    if CheckSea(1) then
        minLv, maxLv = 0, 725
    elseif CheckSea(2) then
        minLv, maxLv = 700, 1500
    else
        minLv, maxLv = 1450, math.huge
    end

    -- Aurora: los overrides hardcodeados (FountainQuest / ForgottenQuest)
    -- YA corresponden a la última isla del mar. Se aplican también en
    -- nivel máximo para que el auto-farm vaya a la isla final en Sea 1/2
    -- en vez de quedarse en una quest intermedia.
        ...
        for r, v in pairs(Quests) do
            for id, v1 in pairs(v) do
                ...
                for nguoi, tinh in pairs(v1.Task) do
                    if lvl >= LvReq and LvReq >= minLv and LvReq <= maxLv
                        and LevelReq <= LvReq and v1.Task[nguoi] > 1
                        and not table.find(blacklistquest, r) then
                        LevelReq = LvReq
                        mmb["Mob"] = nguoi
                        mmb["NameQuest"] = r
                        mmb["ID"] = id
...
local redzlib = loadstring(game:HttpGet("https://api.aurorax.site/v3/loaders/redz-green.lua"))()
local Window = redzlib:MakeWindow({
    Title = "Aurora X | Bloxfruit",
    SubTitle = "https://discord.gg/vuAQtRAAc5",
...
local MobileButtonIcon = "rbxassetid://107063832232126"
local MobileButtonThumb = "rbxthumb://type=Asset&id=107063832232126&w=420&h=420"
...
task.spawn(function()
  ...
  local corner = MobileButton:FindFirstChildOfClass("UICorner")
  if not corner then
...
local Tabs = {
    Info = Window:MakeTab({ Title = "information", Icon = "rbxassetid://90062701178064" }),
    ...
    Quests = Window:MakeTab({ Title = "Quest | Items", Icon = "rbxassetid://84351910860448" }),
    SeaEvent = Window:MakeTab({ Title = "Sea Event", Icon = "rbxassetid://125506289461098" }),
    Race = Window:MakeTab({ Title = "Race V4", Icon = "rbxassetid://73117522660789" }),
    Prehistoric = Window:MakeTab({ Title = "Volcano Event", Icon = "rbxassetid://79414527368822" }),
    Esp = Window:MakeTab({ Title = "Stats,Esp", Icon = "rbxassetid://7040410130" }),
    FruitStock = Window:MakeTab({ Title = "Fruits | Check Stock", Icon = "rbxassetid://77926453204867" }),
    ...
    Player = Window:MakeTab({ Title = "Player & Aimbot", Icon = "rbxassetid://7734053495" })
}
...
Name = "Walk Speed",
Description = "Toggle to override walk speed",
Default = false,
...
Tabs.Player:AddSection("Dash Speed")

Tabs.Player:AddToggle({
Name = "Dash Speed",
Description = "Boost Bloxfruit dash distance",
Default = false,
Callback = function(Value)
  _G.BloxDashBoost = Value
end
})

Tabs.Player:AddSlider({
Name = "Dash Velocity",
Description = "Higher = longer dash",
Min = 1,
Max = 20,
Default = 5,
Callback = function(Value)
  _G.BloxDashPower = Value
end
})

Tabs.Player:AddSection("Aimbot")

Tabs.Player:AddToggle({
Name = "Aimbot Players",
Description = "Redirect abilities to nearest player in FOV",
Default = false,
Callback = function(Value)
  _G.AimbotP = Value
end
...
Tabs.Player:AddToggle({
Name = "Aimbot NPCs",
Description = "Also target NPC enemies",
Default = false,
Callback = function(Value)
  _G.AimbotN = Value
end
...
Description = "Max reach limit (50-5000)",
Min = 50,
Max = 5000,
Default = 3000,
Increment = 50,
Callback = function(Value)
  _G.AimbotMaxDist = Value
end
})
...
Default = "Mouse",
Callback = function(Value)
  _G.AimbotFOVMode = Value
end
})

Tabs.Player:AddSection("Defense")

Tabs.Player:AddToggle({
Name = "Anti-Freeze",
Description = "Stay unfrozen even when hit by stun/freeze abilities",
Default = false,
Callback = function(Value)
  _G.AntiFreeze = Value
end
...
-- === WALK SPEED (BodyVelocity boost while moving) ===
task.spawn(function()
  local RunService = game:GetService("RunService")
  while true do
    ...
    pcall(function()
      ...
      -- Walk Speed: BodyVelocity boost while moving
      if _G.WalkSpeed and _G.WalkSpeedValue and hum.MoveDirection.Magnitude > 0 then
        local speed = _G.WalkSpeedValue * 1.2
        local bv = hrp:FindFirstChild("_AuroraWalkBV")
        if not bv then
          bv = Instance.new("BodyVelocity")
          bv.Name = "_AuroraWalkBV"
      ...
      -- Jump Power
      if _G.JumpPowerValue and _G.JumpPowerValue > 50 then
...
-- === DASH SPEED v7 (hook al boton Dodge real, deslizamiento, respeta energia) ===
task.spawn(function()
  local RunService = game:GetService("RunService")
  local plr = game:GetService("Players").LocalPlayer
  ...
  local function getDashButton()
    local ok, btn = pcall(function()
      return plr.PlayerGui.MobileContextButtons.ContextButtonFrame.BoundActionDodge.Button
    end)
  ...
  local function getEnergy()
    local ok, val = pcall(function()
      return plr.Character.Energy.Value
    end)
    if ok then return val end
    return 0
  end

  -- El boton puede no existir todavia al cargar el script, reintentamos unos segundos
  local dashButton
  for i = 1, 20 do
    dashButton = getDashButton()
    if dashButton then break end
    task.wait(0.5)
  end

  if not dashButton then
    warn("[Aurora Dash] No se encontro el boton Dodge, el Dash Boost no va a funcionar")
    return
  end
  ...
  dashButton.Activated:Connect(function()
    if not _G.BloxDashBoost or onCooldown then return end
    if getEnergy() < MIN_ENERGY then return end
    ...
    task.spawn(function()
      pcall(function()
        ...
        local lastDir = hum.MoveDirection
        if lastDir.Magnitude < 0.1 then
          lastDir = hrp.CFrame.LookVector
        end
        lastDir = Vector3.new(lastDir.X, 0, lastDir.Z).Unit

        local power = _G.BloxDashPower or 5
        local slideSpeed = power * 20
        local slideDuration = 0.3

        local bv = Instance.new("BodyVelocity")
        bv.Name = "_AuroraDashSlide"
        bv.MaxForce = Vector3.new(math.huge, 0, math.huge)
        bv.P = 5000
        bv.Velocity = lastDir * slideSpeed
        bv.Parent = hrp
        ...
        conn = RunService.Heartbeat:Connect(function()
          ...
          if elapsed >= slideDuration or not bv.Parent or not hrp.Parent then
            if conn then conn:Disconnect() end
            if bv.Parent then bv:Destroy() end
            return
          end
          local curDir = hum.MoveDirection
          if curDir.Magnitude > 0.1 then
            lastDir = Vector3.new(curDir.X, 0, curDir.Z).Unit
          end
          local t = 1 - (elapsed / slideDuration)
          bv.Velocity = lastDir * slideSpeed * t
        end)
      end)
...
-- === AIMBOT LOGIC (target selection + FOV) ===
...
pcall(function()
    ...
    if AimbotFOVCircle then
        ...
        AimbotFOVCircle.Radius = 400
        AimbotFOVCircle.Color = Color3.fromRGB(0, 255, 120)
        AimbotFOVCircle.Filled = false
    end
end)

local AimbotCurTarget = nil
...
local function getAimbotTargets()
    ...
    if not mr then return t end
    local maxDist = _G.AimbotMaxDist or 3000
    if _G.AimbotP then
        for _, p in pairs(game:GetService("Players"):GetPlayers()) do
            if p ~= plr then
                ...
                if r and h and h.Health > 0 then
                    ...
                    if d <= maxDist then
                        table.insert(t, {p = r, name = p.Name})
                    end
                end
            end
        end
            ...
            for _, v in pairs(en:GetChildren()) do
                if v:IsA("Model") then
                    ...
                    if r and h and h.Health > 0 then
                        local d = (mr.Position - r.Position).Magnitude
                        if d <= maxDist then
                            table.insert(t, {p = r, name = v.Name})
                        end
                    end
                end
            end
...
local function nearestInFov(targets)
    local best, bd = nil, _G.AimbotFOV or 400
    local cam = workspace.CurrentCamera
    local uis = game:GetService("UserInputService")
    local fp
    if (_G.AimbotFOVMode or "Mouse") == "Mouse" then
        fp = uis:GetMouseLocation()
    else
        fp = Vector2.new(cam.ViewportSize.X / 2, cam.ViewportSize.Y / 2)
    end
    for _, v in ipairs(targets) do
        local sp, on = cam:WorldToViewportPoint(v.p.Position)
        if on then
            local d = (fp - Vector2.new(sp.X, sp.Y)).Magnitude
            if d < bd then
                bd = d
                best = v
            end
...
-- Update MousePos + FOV circle + ESP Line each frame
game:GetService("RunService").RenderStepped:Connect(function()
    pcall(function()
        local active = _G.AimbotP or _G.AimbotN
        if not active then
            AimbotCurTarget = nil
        ...
        if AimbotFOVCircle then
            local uis = game:GetService("UserInputService")
            local cam = workspace.CurrentCamera
            if _G.AimbotShowFOV ~= false then
                if (_G.AimbotFOVMode or "Mouse") == "Mouse" then
                    AimbotFOVCircle.Position = uis:GetMouseLocation()
                else
                    AimbotFOVCircle.Position = Vector2.new(cam.ViewportSize.X / 2, cam.ViewportSize.Y / 2)
                end
                AimbotFOVCircle.Radius = _G.AimbotFOV or 400
        ...
        -- Target selection PRIMERO (antes se calculaba despues de usarlo, por eso no andaba la linea)
        local targets = getAimbotTargets()
        local t = nearestInFov(targets)
        AimbotCurTarget = t
        if t then
            MousePos = t.p.Position
        else
            MousePos = nil
        end

        -- Rainbow ESP Line al objetivo actual del aimbot (jugador o NPC, el que este activo)
        -- Solo se oculta si el objetivo esta detras del personaje (para que no se vea atraves de tu skin)
        if AimbotLine then
            if t and t.p then
                local cam = workspace.CurrentCamera
                ...
                if myHRP then
                    -- Verificar si el objetivo esta DELANTE de la camara (no detras del personaje)
                    local toTarget = (t.p.Position - cam.CFrame.Position).Unit
                    local dotProduct = toTarget:Dot(cam.CFrame.LookVector)
                    if dotProduct > 0 then
                        -- Objetivo delante de la camara: mostrar linea normal
                        local mySP, onMe = cam:WorldToViewportPoint(myHRP.Position)
                        local tgtSP, onTgt = cam:WorldToViewportPoint(t.p.Position)
                        if onMe and onTgt then
                            AimbotLine.From = Vector2.new(mySP.X, mySP.Y)
                            AimbotLine.To = Vector2.new(tgtSP.X, tgtSP.Y)
                            rainbowHue = (rainbowHue + 0.02) % 1
                            AimbotLine.Color = Color3.fromHSV(rainbowHue, 1, 1)
                            AimbotLine.Visible = true
                        else
                            AimbotLine.Visible = false
                        end
                    else
                        -- Objetivo detras del personaje: ocultar linea (no se vea atraves de tu skin)
                        AimbotLine.Visible = false
                    end
                else
                    AimbotLine.Visible = false
...
-- Hide sea-specific tabs when not in the matching sea.
-- Sea Event  -> Sea 2 and Sea 3
-- Race V4    -> Sea 3
-- Volcano    -> Sea 3
local function _AuroraHideTab(wt)
    if not wt then return end
    local raw = wt._tab or wt
    if raw and raw.Button then raw.Button.Visible = false end
end
if not (sea2 or sea3) then _AuroraHideTab(Tabs.SeaEvent) end
if World1 then
    _AuroraHideTab(Tabs.Race)
end
if not sea3 then
    _AuroraHideTab(Tabs.Prehistoric)
end
...
if sea3 then Tabs.Main:AddSection("Rip Indra (Raid)")
...
function Check_Eye()
    ...
    if not tikiOutpost or not tikiOutpost:FindFirstChild("IslandModel") then return 0, false end
    local e = tikiOutpost.IslandModel
    local eyes = {
        e.Eye1,
        e.Eye2,
...
if sea3 then Tabs.Quests:AddSection("Tushita + Yama")
...
if sea2 then Tabs.Quests:AddSection("Rengoku")
Q = Tabs.Quests:AddToggle({
Name = "Auto Rengoku", 
Description = "", 
Default = false,
    ...
    pcall(function()
      ...
      if not GetBP("Dark Blade") then replicated.Remotes.CommF_:InvokeServer("LoadItem","Dark Blade") end
        if GetBP("Fist of Darkness") then
          ...
          elseif GetConnectionEnemies("Darkbeard") and GetBP("Fist of Darkness") then
...
if sea3 then Tabs.Quests:AddSection("Cavander / Twin Hooks / Bigmom")
Q = Tabs.Quests:AddToggle({
Name = "Auto Bigmom", 
Description = "", 
Default = false,
...
if sea1 then Tabs.Quests:AddSection("Buso Haki / Aura Colors")
Q = Tabs.Quests:AddToggle({
...
spawn(function()
  while wait(Sec) do
    if _G.Snaguine then
      pcall(function()
        if not GetBP("Sanguine Art") then replicated.Remotes.CommF_:InvokeServer("BuySanguineArt") end
...
if sea3 then Tabs.SeaEvent:AddSection("Kitsune Island")
...
-- ================== Aurora ESP Player v5 (foto + barra vida + DisplayName) ==================
local ThumbCache = {} -- cache de fotos de perfil para no pedirlas repetido
...
local ESP_BASE_W, ESP_BASE_H = 128, 44
    ...
    for _, v in next, game.Players:GetChildren() do
        pcall(function()
            if not isnil(v.Character) and v ~= plr then
                ...
                if PlayerEsp and head and hum then
                    if not head:FindFirstChild('NameEsp'..Number) then
                        ...
                        bill.Size = UDim2.new(0, ESP_BASE_W, 0, ESP_BASE_H)
                        bill.StudsOffset = Vector3.new(0, 2.4, 0)
                        bill.MaxDistance = math.huge

                        local pic = Instance.new('ImageLabel', bill)
                        pic.Name = 'PlayerPic'
                        pic.Size = UDim2.new(0, 26, 0, 26)
                        pic.Position = UDim2.new(0, 0, 0, 0)
                        pic.BackgroundColor3 = Color3.fromRGB(20, 20, 25)
                        pic.BackgroundTransparency = 0.15
                        pic.BorderSizePixel = 0
                        pic.Image = GetThumb(v.UserId)
                        Instance.new('UICorner', pic).CornerRadius = UDim.new(1, 0)
                        local picStroke = Instance.new('UIStroke', pic)
                        picStroke.Thickness = 1.2

                        local name = Instance.new('TextLabel', bill)
                        name.Name = 'PlayerName'
                        name.BackgroundTransparency = 1
                        name.Size = UDim2.new(1, -32, 0, 12)
                        ...
                        dist.Size = UDim2.new(1, -32, 0, 11)
                        dist.Position = UDim2.new(0, 32, 0, 11)
                        ...
                        -- Barra de vida (un poco mas corta que la linea de arriba)
                        local hpBack = Instance.new('Frame', bill)
                        hpBack.Name = 'HPBack'
                        hpBack.Size = UDim2.new(1, -44, 0, 6)
                        hpBack.Position = UDim2.new(0, 32, 0, 24)
                        hpBack.BackgroundColor3 = Color3.fromRGB(15, 15, 18)
                        hpBack.BackgroundTransparency = 0.1
                        ...
                        local hpText = Instance.new('TextLabel', hpBack)
                        hpText.Name = 'HPText'
                        hpText.BackgroundTransparency = 1
                        hpText.Size = UDim2.new(1, 0, 1, 0)
                        hpText.Font = ESP_FONT
                        hpText.TextSize = 7
                    ...
                    if bill then
                        ...
                        local hpBack = bill:FindFirstChild('HPBack')

                        local isAlly = v.Team and TeamSelf and v.Team == TeamSelf

                        if name then
                            local shownName = v.DisplayName
                            if not shownName or shownName == "" then shownName = v.Name end
                            name.Text = shownName
                            name.TextColor3 = isAlly and Color3.fromRGB(60, 140, 255) or Color3.fromRGB(255, 60, 60)
                        end
                        ...
                        if dist then
                            ...
                            local rc = FullRainbow()
                            local hex = string.format("#%02X%02X%02X", rc.R*255, rc.G*255, rc.B*255)
                            dist.Text = string.format(
                                '<font color="%s">%dm</font>  <font color="#FF7FE0">◆</font>  <font color="#FFD24C">Lv %s</font>',
                                hex, math.floor(d), lvlValue
                            )
                        end

                        if hpBack then
                            local hpFill = hpBack:FindFirstChild('HPFill')
                            local hpText = hpBack:FindFirstChild('HPText')
                            if hpFill then
                                local pct = math.clamp(hum.Health / math.max(hum.MaxHealth, 1), 0, 1)
                                hpFill.Size = UDim2.new(pct, 0, 1, 0)
                                local hpColor
                                if pct > 0.5 then
                                    hpColor = Color3.fromRGB(255, 255 * (2 - pct * 2), 60):Lerp(Color3.fromRGB(60, 220, 100), (pct - 0.5) * 2)
                                else
                                    hpColor = Color3.fromRGB(220, 60, 60):Lerp(Color3.fromRGB(255, 210, 60), pct * 2)
                                end
...
-- ================== Aurora ESP Fruit v1 (simple text, version antigua) ==================
DevEsp = function()
            ...
            if DevilFruitESP then
                if string.find(v.Name, "Fruit") and v:FindFirstChild("Handle") then
                    ...
                    if not handle:FindFirstChild('NameEsp'..Number) then
                        ...
                        name.BackgroundTransparency = 1
                        name.TextStrokeTransparency = 0.5
                        name.TextColor3 = Color3.fromRGB(255, 130, 235)
                        name.Text = ("Fruit: " .. v.Name .. " \n" .. round((plr.Character.Head.Position - handle.Position).Magnitude / 3) .. " M")
                    else
                        handle['NameEsp'..Number].TextLabel.Text = ("Fruit: " .. v.Name .. " \n" .. round((plr.Character.Head.Position - handle.Position).Magnitude / 3) .. " M")
                        handle['NameEsp'..Number].TextLabel.TextColor3 = MonoRainbow(HUE_FRUIT)
                        handle['NameEsp'..Number].TextLabel.Font = ESP_FONT
                    end
                end
            else
...
Tabs.FruitStock:AddSection("Fruit Stock")
...
local stockParagraph = Tabs.FruitStock:AddParagraph("Stock Fruit", "Loading...")
...
Tabs.FruitStock:AddSection("Option Fruits")

RandomFF = Tabs.FruitStock:AddToggle({
...
DropF = Tabs.FruitStock:AddToggle({
...
StoredF = Tabs.FruitStock:AddToggle({
        ...
        pcall(function()
            if getgenv().AutoGetFruit then
                local fruits = {
                    ...
                    "Spike-Spike", "Flame-Flame", "Falcon-Falcon", "Eagle-Eagle", "Ice-Ice", "Sand-Sand", "Dark-Dark",
                    "Ghost-Ghost", "Diamond-Diamond", "Light-Light", "Rubber-Rubber", "Barrier-Barrier", "Creation-Creation"
                }
...
Location = {}
local IslandRenames = {
  ["l'Église de Prophétie"] = false,
  ["l'Eglise de Prophetie"] = false,
  ["???"] = false,
  ["Sea"] = false,
  ["Swan's Room"] = false,
  ["Swans Room"] = false,
  ...
  ["Mansión"] = "Flamingo Mansion",
  ["Mansion"] = "Flamingo Mansion",
}
local LocationTargets = {}
local seenGraveyard = false
local FlamingoRoomCF = CFrame.new(2284.41, 15.38, 875.72)
local Sea3_TeleportIslands = {
  {name = "Cocoa Island",       cf = CFrame.new(87.94,     73.56,   -12319.46)},
  {name = "Candy Island",       cf = CFrame.new(-1014.42,  149.11,  -14555.96)},
  {name = "Tiki Outpost",       cf = CFrame.new(-16218.68, 9.09,    445.62)},
  {name = "Dragon Dogo",        cf = CFrame.new(5743.32,   1207.05, 936.01)},
  {name = "Ice Cream Island",   cf = CFrame.new(-902.57,   79.74,   -10988.85)},
  {name = "Peanut Island",      cf = CFrame.new(-2062.75,  49.85,   -10232.57)},
  {name = "Cake Island",        cf = CFrame.new(-1850.71,  22.28,   -11680.82)},
  {name = "Hydra Island",       cf = CFrame.new(5291.25,   1005.56, 393.76)},
  {name = "Floating Turtle",    cf = CFrame.new(-13274.53, 531.82,  -7579.22)},
  {name = "Haunted Castle",     cf = CFrame.new(-9515.37,  164.21,  5786.06)},
  {name = "Mansion",            cf = CFrame.new(-12471.16, 375.15,  -7551.65)},
  {name = "Port Town",          cf = CFrame.new(-226.75,   20.89,   5538.34)},
  {name = "Great Tree",         cf = CFrame.new(2681.27,   1682.80, -7190.98)},
  {name = "Castle On The Sea",  cf = CFrame.new(-5083.26,  314.74,  -3175.67)},
}
if World3 then
  for _, ex in ipairs(Sea3_TeleportIslands) do
    if not LocationTargets[ex.name] then
      table.insert(Location, ex.name)
      LocationTargets[ex.name] = ex.cf
    end
  end
else
for _, v in pairs(workspace["_WorldOrigin"].Locations:GetChildren()) do
  local orig = v.Name
  local display = orig
  if IslandRenames[orig] ~= nil then
    if IslandRenames[orig] == false then
      display = nil
    else
      display = IslandRenames[orig]
    end
  end
  if orig == "Graveyard Island" then
    if seenGraveyard then
      display = nil
    else
      display = "Zombie Island"
      seenGraveyard = true
    end
  end
  if display and not LocationTargets[display] then
    table.insert(Location, display)
    LocationTargets[display] = v
    if display == "Flamingo Mansion" then
      table.insert(Location, "Flamingo room")
      LocationTargets["Flamingo room"] = FlamingoRoomCF
    end
  end
end
end
if World2 then
  local ExtraSea2 = {
    {name = "Dark Arena",   cf = CFrame.new(3780.02, 22.89, -3498.56)},
    {name = "Frist Spot",   cf = CFrame.new(-11.31, 29.48, 2771.52)},
    {name = "Ussop Island", cf = CFrame.new(4816.86, 8.66, 2863.82)},
  }
  for _, ex in ipairs(ExtraSea2) do
    if not LocationTargets[ex.name] then
      table.insert(Location, ex.name)
      LocationTargets[ex.name] = ex.cf
    end
  ...
  if Value then
    task.spawn(function()
      while _G.Teleport do
        local isle = LocationTargets[_G.Island]
        if isle then
          local destPos
          if typeof(isle) == "CFrame" then
            destPos = isle.Position
          elseif typeof(isle) == "Instance" then
            destPos = isle.CFrame.Position
          end
          if destPos then
            local plr = game:GetService("Players").LocalPlayer
            local char = plr.Character
            ...
            if hrp then
              ...
              while _G.Teleport do
                ...
                local pos = startPos:Lerp(destPos, t)
                local flyH = height
                if t < 0.1 then flyH = height * (t / 0.1)
                elseif t > 0.8 then
                  flyH = 30 + (height - 30) * ((1 - t) / 0.2)
                end
                hrp.CFrame = CFrame.new(pos.X, pos.Y + flyH, pos.Z)
                -- Sync block to prevent follow-loop rubber-band
                pcall(function() if block then block.CFrame = hrp.CFrame end end)
                task.wait(0.04)
              end
              if _G.Teleport then
                old_tp(CFrame.new(destPos.X, destPos.Y + 50, destPos.Z))
              end
            end
          end
        end
        if not _G.Teleport then break end
        local last = _G.Island
        repeat task.wait(0.3) until _G.Island ~= last or not _G.Teleport
      end
    end)
  end
end
...
local function StartMainLoops()
    ...
    -- Aurora patch: re-enganchar folders y reintentar tras cada respawn
    LocalPlayer.CharacterAdded:Connect(function(char)
        pcall(function()
            char:WaitForChild("HumanoidRootPart", 10)
            Enemies = SafeWaitForChild(Workspace, "Enemies") or Enemies
            Characters = SafeWaitForChild(Workspace, "Characters") or Characters
        end)
    end)
end
...
-- =============================================================
-- Aurora sea-gating: hide sections that don't belong to the
-- current sea (Update 26 mapping). Uses per-section tracking
-- added in the AuroraUI_redz shim so widgets under a hidden
-- section are also hidden.
-- =============================================================
do
    local S1, S2, S3 = sea1, sea2, sea3
    local function isAllowed(list)
        for _, s in ipairs(list) do
            if (s == 1 and S1) or (s == 2 and S2) or (s == 3 and S3) then
                return true
            end
        end
        return false
    ...
    -- { tab, "Section Name", { seas allowed } }
    local gates = {
        -- Main tab
        { Tabs.Main, "Elite Hunter",           {2, 3} },
        { Tabs.Main, "Rip Indra (Raid)",       {3} },
        { Tabs.Main, "Cake Event",             {3} },
        { Tabs.Main, "Bones Event",            {3} },
        { Tabs.Main, "Tyrant of the Skies",    {3} },
        -- Quests tab
        { Tabs.Quests, "Tushita + Yama",                {3} },
        { Tabs.Quests, "Skull Guitars",                 {3} },
        { Tabs.Quests, "Cursed Dual Katana",            {3} },
        { Tabs.Quests, "True Triple Katana",            {3} },
        { Tabs.Quests, "Rengoku",                       {2} },
        { Tabs.Quests, "Cavander / Twin Hooks / Bigmom",{3} },
        { Tabs.Quests, "Buso Haki / Aura Colors",       {1} },
        { Tabs.Quests, "Observation Haki",              {3} },
        -- Race tab (Sea 3 tab is already hidden outside sea3)
        { Tabs.Race, "Mirage Island",         {3} },
        { Tabs.Race, "Awaken Race V2 / V3",   {2, 3} },
        { Tabs.Race, "Race V4 Trials",        {3} },
        -- Prehistoric / Volcano tab (already hidden outside sea3, but gate for safety)
        { Tabs.Prehistoric, "Dojo Quest",         {3} },
        { Tabs.Prehistoric, "Draco Trial",        {3} },
        { Tabs.Prehistoric, "Volcanic Crafting",  {3} },
        { Tabs.Prehistoric, "Prehistoric Island", {3} },
        -- Sea Event tab (tab is already hidden outside sea2/sea3)
        { Tabs.SeaEvent, "Sea Events",        {2, 3} },
        { Tabs.SeaEvent, "Item Crafting",     {2, 3} },
        { Tabs.SeaEvent, "Choose Sea Event",  {2, 3} },
        { Tabs.SeaEvent, "Farm Sea Event",    {2, 3} },
        { Tabs.SeaEvent, "Kitsune Island",    {3} },
        { Tabs.SeaEvent, "Frozen Dimension",  {3} },
        -- Raids tab
        { Tabs.Raids, "Dungeon Event", {2, 3} },
        { Tabs.Raids, "Raid Law",      {3} },
        -- Shop tab
        { Tabs.Shop, "Buy Melee V1", {1} },
        { Tabs.Shop, "Buy Melee V2", {2, 3} },
    }

    for _, g in ipairs(gates) do
        local tab, name, seas = g[1], g[2], g[3]
        if tab and tab._sections and not isAllowed(seas) then
            for _, sec in ipairs(tab._sections) do
                if sec.Name == name and sec.Hide then
                    sec:Hide()
                end
            end
        end
    ...
    -- Per-widget gating: weapon / puzzle / auto-obtain toggles that live
    -- inside a shared section but only make sense in one sea.
    local widgetGates = {
        -- Quest Sword section (mixed Sea 1 / Sea 2)
        { Tabs.Quests, "Auto Pole V1",       {1} },
        { Tabs.Quests, "Auto Pole V2",       {1} },
        { Tabs.Quests, "Auto Saw Sword",     {1} },
        { Tabs.Quests, "Auto Saber Sword",   {1} },
        { Tabs.Quests, "Auto Cybrog",        {2} },
        { Tabs.Quests, "Auto Usopp's Hat",   {2} },
        { Tabs.Quests, "Auto Bisento V2",    {2} },
        { Tabs.Quests, "Auto Warden Sword",  {2} },
        { Tabs.Quests, "Auto Marine Coat",   {2} },
        { Tabs.Quests, "Auto Swan Coat",     {2} },
        -- Buy Sword / Gun shop entries by sea availability
        { Tabs.Shop,   "Buy Bisento",        {2} },
        { Tabs.Shop,   "Buy Pipe",           {1} },
        { Tabs.Shop,   "Buy Dual Katana",    {1} },
        { Tabs.Shop,   "Buy Iron Mace",      {1} },
        { Tabs.Shop,   "Buy Triple Dark Blade", {1} },
        { Tabs.Shop,   "Buy Cutlass",        {1} },
        { Tabs.Shop,   "Buy Katana",         {1} },
        { Tabs.Shop,   "Buy Dragon Trident", {3} },
        { Tabs.Shop,   "Buy Soul Cane",      {3} },
        { Tabs.Shop,   "Buy Midnight Blade", {3} },
        { Tabs.Shop,   "Buy Buddy Sword",    {3} },
        { Tabs.Shop,   "Buy Longsword",      {2} },
        { Tabs.Shop,   "Buy Pole (1st Form)", {1} },
        { Tabs.Shop,   "Buy Musket",         {1} },
        { Tabs.Shop,   "Buy Slingshot",      {1} },
        { Tabs.Shop,   "Buy Flintlock",      {1} },
        { Tabs.Shop,   "Buy Refined Slingshot", {2} },
        { Tabs.Shop,   "Buy Refined Flintlock", {2} },
        { Tabs.Shop,   "Buy Cannon",         {2} },
        { Tabs.Shop,   "Buy Kabucha",        {3} },
        { Tabs.Shop,   "Buy Serpent Bow",    {3} },
        { Tabs.Shop,   "Buy Bazooka",        {3} },
    }
    for _, g in ipairs(widgetGates) do
        local tab, name, seas = g[1], g[2], g[3]
        if tab and tab.HideWidget and not isAllowed(seas) then
            tab:HideWidget(name)
        end
    end
end
